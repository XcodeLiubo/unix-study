...
### glibc的TCB
- 线程最核心的功能一定由内核实现, glibc属用户库函数, 所以它必定依赖内核. 内核描述调度单位(`线程`)有TCB的概念, 位于上层glibc在封装时, 也会定义自己层面的TCB来映射到内核(<font color=green>就像stdio的文件流对应内核的文件描述符</font>), 这里只学习glibc层面的TCB, 但事实上是和内核中定义的相对应. 这里整理出来的结构如下

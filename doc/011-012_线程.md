...
static int create_thread (  struct pthread *pd, 
    ...
    作用:它用来标记新线程(pd)启动需要做同步

    在具体说明它的作用前先来看if的2个条件:
        1. 指定了cpu的亲和力
...
static int _Noreturn start_thread (void *arg){
  struct pthread *pd = arg;
    ...
    有这样一种情况:
        当 create_thread --> clone后, 还没从clone返回, 新线程已经运行了
        此时 以 create_thread的角度来看, 它并不知道 clone是否调用成功
         

    因为可能会出现这样的情况. 再加上 stopped_start 为true时必须
    ...
    ps: 这里有个疑问, 即若停止标记被设置, 为什么在启动线程检查时,直接结束
            goto out; 
        后并没有释放任何相关的资源, 如TCB, 栈, TLS, TSD等?
#endif
  // 只有当新线程(pd)
  //    > 指定自定义优先级属性 或 指定了CPU新和力
  // 会标记启动停止
  if (pd->stopped_start)
    {
  ...
  if (__glibc_likely (! not_first_call)){
    ...
    // 用户函数返回, 执行 pthread_cleanup_push函数
#ifndef SHARED
...
# endif
  ...
  if (IS_DETACHED (pd))
    ...
    所以调用 exit所发送的信号 CLONE_CHILD_CLEARID 猜测应该由
    C的运行库(CRT)内部做处理, 发现当前线程(pd)的 set_failed 标记为false, 会
    在那个地方清理

    glibc应该是在调用main函数之前注册了 CLONE_CHILD_CLEARID信号处理
#endif
out:

    // 这里不能使用 _exit(系统调用)退出, 而应该使用 exit(标准库)
    // 内核支持exit的实现, 会发送 CLONE_CHILD_CLEARID这个信号
    // 总之这是glibc的解释
  while (1)
...
### glibc的其他属性(`略`)
- 在书籍中, 只是简单的给出了相关的接口(<font color=green>如读写锁, 屏障,条件变量等</font>), 这里分析的过程和上面源码中一样, 这里就不多说了, 具体看相关的接口

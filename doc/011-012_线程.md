    ...
    do{
        while (1){
            ...
            这里 if 的2个条件逻辑很严密(下面简称 if-1, if-2):
                if-2 成立的场景: Thread_a醒来并唤醒 __g_refs[1]上的Thread_b, Thread_b执行了第5步后, Thread_a才来到这里, 发现
                __g_start的值已经累加, 并且大于当前线程以前看到的 wseq, 从逻辑上应该说明第1组必须被处理完, 所以也是直接返回

            这里并不明白 if-1 是在什么条件下成立的
#endif
...
#if 0
    这一段就是在等待上一组全部处理完毕(g1是传递进来的, 是反转的表示上一组)
#endif

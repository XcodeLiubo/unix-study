...
- 线程的私有数据也称为 <font color=deeppink>线程的特定数据</font>(`thread-specific data`). 这部分的数据是线程独有的. 编译器常常用的`thread(C) thread_local(c++)`关键字实际上是编译器在编译时期调用了相关接口. 它是存储和查询某个线程相关联数据的一种机制.

> 在实际发型中, 线程必定存在自己的数据, 从逻辑上来看,这些数据是自己要维护的, 其他的线程不应该访问到. 所以实现这种机制 1. 利用线程ID的唯一性(<font color=green>进程中</font>)做数组映射, 但不现实(<font color=green>不连续, 不同平台实现不同</font>). 2. 操作系统提供其他的支持. 因为pthread是线程库, 所以这种机制由库提供(如glibc)
>
> <br/>
> 
...
### 线程变量
- 通过`_Thread_local`修饰变量可以在C环境下由编译器创建一个线程变量(`C++环境下通过Thread_local`). 根据前面的学习,事实上这种变量就是TSD. 下面分别在MacOS和Linux上作出验证
...
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
  * frame #0: 0x00007ff811b145c6 libdyld.dylib`dyld4::LibSystemHelpers::pthread_setspecific(unsigned long, void const*) const
    ...
    frame #2: 0x00007ff811b04555 libdyld.dylib`tlv_get_addr + 296
...
> PS: 需要说明的是本人的配置是<font color = red>M2架构</font>(`arm64`), 但在shell调试中的汇编指令却是`x86`, 原因和M2芯片上的Rosetta有关.在接下来的Linux中汇编指令是`arm`, 会有区别, 这里说明一下

<br/>

> 若是C++是一样的过程. 说明在MacOS环境下, 线程变量的本质就是编译器在运行时将`_Thread_Local`所修饰的变量转变成相关的接口调用. <font color=red>事实上这种TSD本质存储在TCB结构中的某个字段, 这个在后面的Linux环境下作出源码说明</font>
> 接下来看Linux环境下
...
Breakpoint 1, main (args=1, argv=0xfffffffffb08) at main.c:7
7		__auto_type ptr = &a;                   # 断点在  __code_0
(gdb) disassemble                               # 查看汇编
Dump of assembler code for function main:
   0x0000aaaaaaaa07cc <+0>:	stp	x29, x30, [sp, #-48]!
   0x0000aaaaaaaa07d0 <+4>:	mov	x29, sp
   0x0000aaaaaaaa07d4 <+8>:	str	w0, [sp, #28]
   0x0000aaaaaaaa07d8 <+12>:	str	x1, [sp, #16]
=> 0x0000aaaaaaaa07dc <+16>:	mrs	x0, tpidr_el0                       ## __shell_1
   0x0000aaaaaaaa07e0 <+20>:	add	x0, x0, #0x0, lsl #12               ## __shell_2
   0x0000aaaaaaaa07e4 <+24>:	add	x0, x0, #0x10                       ## __shell_3
   0x0000aaaaaaaa07e8 <+28>:	str	x0, [sp, #40]                       ## __shell_4
   0x0000aaaaaaaa07ec <+32>:	bl	0xaaaaaaaa0670 <pause@plt>
   0x0000aaaaaaaa07f0 <+36>:	mov	w0, #0x0                   	// #0
   0x0000aaaaaaaa07f4 <+40>:	ldp	x29, x30, [sp], #48
   0x0000aaaaaaaa07f8 <+44>:	ret
End of assembler dump.


### __shell_1到__shell_4对应的是 代码中的 __code_0
### 在linux下(arm64)并未看见明显的pthread_sepecific的跳转(虽然没有手动的去在该函数配置断点, 但实际上测试过程中发现并没有该函数的调用)
### 查看相关的arm指令, tpidr_el0 的指令意义是存储着线程相关的TLS(`thread local store,也就是线程私有数据`)
### 所以这4步的本质和MacOS中一样(x86), 只是过程不一样, x86不支持硬件辅助, 所以必须由编译器来调用ABI接口, arm硬件本身和OS有约定
### 可以直接使用指令快速存储. 但不管怎样, 这些存储的数据空间一定处于TCB中
```
...
- 对于`_Thread_local`变量, 不同的系统实现不一样. 从逻辑上来说这种变量在编译时就应该确定地址, 通过这个说法它应该存在于内存布局中的全局变量区域. 但若是这样就真正变成了`全局变量`, 导致所有线程都可以访问. 所以编译器编译代码时, 我们所书写的`_Thread_local`由编译器通过ABI[^ann-abi-0]将其深复制到了TCB相关的字段中, 并通过特定的机制(`pthread_getspecific或arm64约定的tpidr_el0等`)访问. 这种变量在linux中其实是所谓的`线程本地存储(TLS)`, 在创建TCB的时候就会分配固定的大小(`因为编译器在编译期就可以确定有多少_Thread_local变量并确定大小`), 这个过程就不细说了. 若用户自己调用`pthread_setspecific`存储malloc返回的地址, 则是另一回事. 所以以前说的线程的堆栈是有一定意义的

<br/>

> 注意在Mac上实现的机制是直接调用`pthread_setspecific`而没有所谓的(`TLS thread local store`), 原理也是一样, 一定会复制一份到TCB中

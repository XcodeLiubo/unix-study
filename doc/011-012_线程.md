...
- 信号是异步[^ann-async]的, 信号未决前当前线程是暂停的, 当信号由内核制造出来时会随机派发到某个线程. 但有些硬件故障的信号并不是随机的, 会定位到产生这个异常的线程中去
...
>
> Linux中出现这种效果的原因可能是: 响应信号的线程可能被缓存, 下次同样信号直接从缓存中取它, 当然该线程虽然在缓存中, 但也是暂停的. 这纯属个人猜测


### 信号只会暂停当前线程
- 一直以来笔者始终认为当硬件中断发生到信号产生, 再到派发处理前, 进程中所有的线程都是暂停状态. 但这里经过测试发现只有当前线程会被暂停, 其他线程并不影响
...
> 上述程序有2个线程在运行时, 都将处于死循环. 当`SIGINT`发生时, 内核会将信号指定到进程中的某个线程执行, 当线程调用处理函数时, 直接进入了暂停状态. 通过测试可以发现, 当发生第1次信号时, 2条线程中只有处理信号的线程进入了暂停. 另1条还在占据cpu. 当第2次信号发生时, 内核只能将信号交给另1条线程, 接着该线程执行信号处理后, 也进入了暂停
...
#### 摁下ctrl + c后,主线程会响应一次后, 暂停, 然后继续键入相同的按键, 程序不会有任何线程响应
/SUB_THREAD.out
pid	[	19740	]
...
^C
hand	[	281473440108992	]   # 子线程响应后永久挂起(不响应其他信号)
^C^C^C^C
...
[^ann-async]: 这里所说的异步不同于并发. 以信号来说, 当产生硬件中断时, 当前CPU收到中断请求, 暂停当前的任务(`当前线程`) 去查找中断向量表, 然后处理, 信号就是在内核处理程序中生成的信号序列, 然后转发到当前线程注册的信号处理函数. 也就是说当信号发生时, 当前线程一定处于暂停状态, 等到被调度时, 会进入到信号处理阶段. 并发是所有的线程都在工作. 注意一点, 多核下, 信号发生时, 只有当前cpu所在的线程才会暂停, 其他线程并不会暂停

...
int pthread_create(pthread_t* newthread, const pthread_attr_t* attr, Start_routine start_routine, void* arg){
...
#if 0
    判断是不是单线程模式, 该变量是一个全局的标识, 若为1则一定表示此刻还是单线程
    用户调用该函数目的是为了创建线程, 不管成不成功, 都会将该值置0,
    所以该值为0并不表示当前是多线程模式

    late_init():
        注册 SIGCANCEL 和 SIGSETXID 2个信号

        SIGCANCEL 是线程库内部使用的信号, 值是 SIGRTMIN - 2(32), 不处于标准信号和时时信号
        SIGSETXID 是33, 同SIGCANCEL一样不属于标准信号和时时信号
#endif
...
#if 0
     绝大多数使用的场景是: 用户传递的 attr为 NULL或者自己创建了 attr
     c11实际的值是-1, 用来标记线程模型是不是 C11标准
     这里多数情况下 c11的值为false

    若用户不设置 attr, 则会拷贝默认的属性
#endif
    ...
    // 初始化TCB的栈信息
    // 为TCB分配栈, 但栈相关的信息可能由用户指出(iattr)
    // 若用户提供了 stackaddr, 则内部会在此地址上分配栈,并做调整
    // 若用户未提供 stackaddr, 则内部会申请新的内存做为线程的栈
    ...
    *newthread = (pthread_t) pd;
...
#if 0
    根据glibc的解释, 这里必须在创建线程前(现在还未真正系统调用clone), 由当前线程来
    完成 线程计数器的增1, 因为若是让新线程(pd)来做这个操作, 则可能当前线程在调用完clone
    后还未返回, 在新线程看来自己是唯一的线程, 这是一种错误的逻辑

    所以当系统调用clone创建进程后, 若失败, 一定会对该计数器-1

    ps: Linux内核中没有线程的概念, clone的系统调用指定不同的参数会创建一种轻量级的进程, 可以直接理解为线程
#endif
    ...
    // 这里前后阻塞和恢复有这样的意义:
    //      调用clone创建线程的过程不被任何信号打断, 因为clone是系统调用, 若被信号打断, 可能会重启, 但创建线程的效率会降低
    //          所以屏蔽所有信号, 会加快线程的创建
...
/**
    ...
    函数的过程:
        1. 第1次调用该函数时, 注册 SIGCANCEL和SIGSETXID信号(时时)
        2. 复制当前线程属性, 记录线程启动函数和参数
        3. 分配TCB(pd)的栈空间
...
/*
    glibc在设置锁的类型时, 内部做的转换
        NOR:    1111 0000 1111 1111 1111 0001 0000 0000    
...
static void common(){
    ...
    id = shmget(key, sizeof(pthread_mutex_t), O_CREAT | 0666);

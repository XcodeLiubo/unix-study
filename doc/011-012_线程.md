...
static __always_inline int __pthread_rwlock_rdlock_full64 (
    ...
    while的条件意味着 glibc在处理读者数量时, 并不是简单的用32位(int)去记录 读者数量
        ...
        第1位( PTHREAD_RWLOCK_WRLOCKED:2): 准备写
            阻塞在 __data.__wrphase_futex等待解锁函数唤醒, 该状态可能由 等待写唤醒,
            然后发现有此刻有正在写, 自己则变为准备写
        ...
        整个最外层 while 条件要根据 wrlock函数来解释:
            ...
            而这里的 while 表示:
                存在准备写(它在等待正在读的线程完毕) 并且 有其他读者,
                则当前请求的读操作不能立即返回(不能变为正在读),
                它要先阻塞在 __data.__readers, 等待 正在读的线程唤醒(rdunlock), 然后可能再进入
                到准备读(即醒来后条件不足, 阻塞在 __data.__wrphase_futex)

            说白了就是: 当前读操作时, 发现同时有准备写(也是一种等待), 它不能立即读, 要等当前正在读的线程完毕,
                      这样的目的是为了在 __data.__wrphase_futex这块内存上阻塞的更多的是写线程, 间接的写线程被
                      调度的概率就会增高.

            这就是APUE书籍中所说的, 系统实现读写锁时, 会尝试写优先. glibc仅仅用了2把锁(futex同步的内存)
                就简单高效的实现了 写优先的算法
        ...
        while ((r & PTHREAD_RWLOCK_WRPHASE) == 0
            ...
            if (atomic_compare_exchange_weak_relaxed
                ...
                // 前面标记的过程并不确定
                // 此时还必须看真正的内存中(__data.__readers)是不是真的已经被标记了读等待
                // 若真正的是, 则就等待 abstime 时间
...
#if 0
    代码执行到这里, 一定说明 当前线程所请求的读操作 看到的前一刻的状态是
        没有 等待写(PTHREAD_RWLOCK_WRLOCKED位为1,  PTHREAD_RWLOCK_WRPHASE位为0)
    虽然不能确定此刻 __data.__readers的状态, 但在逻辑上当前线程应该继续往下走

    不管什么情况, 后续的判断都必须再次检查 __data.__readers的状态
...
#if 0
    代码执行到这里, 若当前被唤醒的, 则很大概率该if的条件是不成立的
    若是没经过 等待读, 则该条件很大概率是成立的, 直接返回

    PS: 也说明了 正在读时, 读者的数量并未先-1, -1的操作在 rdunlock函数中
#endif
    ...
    所以这里的操作就是纠正, 发现条件不对强制设置 WRPHASE为0
#endif
    while ((r & PTHREAD_RWLOCK_WRPHASE) != 0
           && (r & PTHREAD_RWLOCK_WRLOCKED) == 0){
        // 使用 异或 强制设置WRPHASE为0
        if (atomic_compare_exchange_weak_acquire (&rwlock->__data.__readers, &r,
            ...
            // 就做一下唤醒操作, 因为可能阻塞的有其他读线程,唤醒它们, 若唤醒的是写线程, 则会
            // 根据条件继续阻塞
...
static __always_inline int __pthread_rwlock_wrlock_full64 (pthread_rwlock_t *rwlock, clockid_t clockid,const struct __timespec64 *abstime){
    ...
    这里就根据用户设置的 读写锁类型有了策略
        当读写锁不是读者优先时(倾向写者), 则要记录写者数量(单独的字段 __data.__writers)
#endif

    // 当前有准备写的线程(也可能有正在写(WRPHASE为1, WRLOCKED为1))
    // 该条件只判断当前有 准备写的线程
        ...
        for (;;){
            ...
            这里是策略性的决定要不要等待(先自旋)
                ...
                从效果上来看说白了就是: 当等待写的数量达到阀值时, 进行新的一轮计数
                    所以 __data.__writers并不是真正计数等待写的数量
            ...
            PS: 前面关于阀值的判断中可能跳出循环, 说白了跳出循环的线程是第1个发现写者数量 > 阀值的线程, 其他
                ...
                出现这种情况的场景是: 前面所述的大量写线程导致计数器激增(如 0xffffffff), 然后有某个线程
                执行到前面的判断阀值部分, 导致计数 0x7ffffffd, 它抢占后再解锁唤醒当前线程前的这段时间, 又多了4条新的写线程
                导致 if(w == PTHREAD_RWLOCK_WRHANDOVER + 1) 成立, 跳出 for 循环, 这就直接忽略了超时
    ...
    一条线程超时醒来, 检查条件可以跳转循环, 此时会来到这里, 但正在写的线程存在
    这条超时的线程直接就返回了
        ...
        while (((wpf = atomic_load_relaxed (&rwlock->__data.__wrphase_futex))
            ...
            if (err == ETIMEDOUT || err == EOVERFLOW){
                ...
                if (rwlock->__data.__flags != PTHREAD_RWLOCK_PREFER_READER_NP){
                    ...
                    // 若有等待写, 这里并未使用 WRLOCKED的位为0时来判断, 其实是没有问题的
                    // 当前 准备写的线程 其WRLOCKED一定为1, 所以它等的一定是 正在读的线程
                    // 当超时醒来时, 因为没有调用 wrunlock, 所以WRLOCKED的值此刻还是1
                    // 现在它要结束, 相当于要执行 wrunlock, 但流程又有所不同
                    // 和wrunlock一样的是, 当锁类型是写优先时, 有等待读时, 也是唤醒它们直接返回
                    if (w != 0){
                ...
                if ((r & PTHREAD_RWLOCK_WRPHASE) == 0){
                    ...
                    while ((r & PTHREAD_RWLOCK_WRPHASE) == 0){
                        ...
                        if (atomic_compare_exchange_weak_release
                            ...
                            // 唤醒 等待写, 其中1个会抢占设置WRLOCKED为1, 后续可能进入正在读
                            if ((wf & PTHREAD_RWLOCK_FUTEX_USED) != 0)
                                futex_wake (&rwlock->__data.__writers_futex,
                                            1, private);

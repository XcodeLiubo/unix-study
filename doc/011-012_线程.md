...
> PS: 当去掉`WR_TO_FILE`宏时, 测试的是多线程生产, 当队列中的任务满了3个后, 生产者会阻塞等待消费者取走任务. `__code_consumer_judge`这里必须使用`while`. 因为有这样一种情况: <font color=red>当consumer1执行到`__code_consumer_judge`时被挂起(`时间片到了`), 然后consumer2执行到`__code_consumer_wait`时被阻塞, 然后下一时刻有一个生产者添加了任务, 并调用了`signal`.  随后调度器挂起所有的生产者. 调度器开始调度消费者, 此时consumer1会因为队列不为空直接跨过if, consumer2会因为前面的生产者的signal被唤醒, 也会往下执行, 2个消费者必有1个取不到任务,导致内存错误, 所以必须在唤醒后再重新检查队列, 不满足时再睡眠</font>
...
### 线程重入
- 在第10章提到过, 当信号发生时会转到信号处理程序, 原来的正常流程的函数被挂起, 所以若在信号处理程序中调用那些处理静态或全局数据结构的函数可能会发生问题(<font color=green>具体看第10章</font>). 所谓的异步可重入就是在信号处理期间所调用的函数是安全的, 大多数的系统调用基本都是异步可重入的. 
...
### 文件IO是线程可重入的
- 对于操作系统提供的系统调用文件IO来说, 它一定是线程安全的, 同时也是可重入的(<font color=green>详见第10章</font>).

> 这里要强调一点: <font color=red>这里的安全是内核在并发状态下操作文件描述符(<font color=green>file descriptor</font>)的文件状态标记(<font color=green>文件表项,详见文件IO</font>)时是安全的, 这样导致并发时对同一个文件描述符作一次性write时是互斥的 </font>
...
int main(int args, char** argv){
    ...
    for(int i = -1; ++i < 50;){
        std::thread([]{
            ...
            memset(buf, 0, 1024);
            snprintf(buf, 1024, "thread[%p]\ttime[%llu]\n", pthread_self(), clock_gettime_nsec_np(CLOCK_REALTIME));
            write(fd, buf, strnlen(buf, 1024));
        }).detach();
    }
    pause();
...
> 上述程序创建了一个共用的文件描述符(<font color=green>fd</font>), 在多线程的并发下, 向关联的文件中写入数据, 最后测试发现, 文件中每一行是完整的, 但并不能确定在写入期间线程的顺序(<font color=green>MacOS平台</font>)

<br/>

```shell
     1	thread[0x16fe87000]	time[1700360833334960000]
     2	thread[0x17025b000]	time[1700360833334989000]
     3	thread[0x1700b7000]	time[1700360833334976000]
     4	thread[0x16ff13000]	time[1700360833334966000]
     5	thread[0x1700b7000]	time[1700360833335057000]
     6	thread[0x170517000]	time[1700360833335064000]
     7	thread[0x17002b000]	time[1700360833334971000]
     8	thread[0x170373000]	time[1700360833335015000]
     9	thread[0x16fe87000]	time[1700360833335029000]
    10	thread[0x1701cf000]	time[1700360833334990000]
    11	thread[0x17002b000]	time[1700360833335113000]
    12	thread[0x1700b7000]	time[1700360833335122000]
    13	thread[0x16ff9f000]	time[1700360833334972000]
    14	thread[0x170143000]	time[1700360833334985000]
    15	thread[0x16fe87000]	time[1700360833335114000]
    16	thread[0x17002b000]	time[1700360833335165000]
    17	thread[0x17048b000]	time[1700360833335048000]
    18	thread[0x1700b7000]	time[1700360833335176000]
    19	thread[0x16ff13000]	time[1700360833335146000]
    20	thread[0x17062f000]	time[1700360833335108000]
    21	thread[0x1705a3000]	time[1700360833335100000]
    22	thread[0x1703ff000]	time[1700360833335027000]
    23	thread[0x16ff9f000]	time[1700360833335157000]
    24	thread[0x1706bb000]	time[1700360833335155000]
    25	thread[0x17025b000]	time[1700360833335041000]
    26	thread[0x1702e7000]	time[1700360833335005000]
    27	thread[0x17025b000]	time[1700360833335262000]
    28	thread[0x16ff13000]	time[1700360833335209000]
    29	thread[0x17002b000]	time[1700360833335208000]
    30	thread[0x16ff9f000]	time[1700360833335286000]
    31	thread[0x170517000]	time[1700360833335109000]
    32	thread[0x170143000]	time[1700360833335203000]
    33	thread[0x16ff13000]	time[1700360833335313000]
    34	thread[0x17025b000]	time[1700360833335319000]
    35	thread[0x17048b000]	time[1700360833335257000]
    36	thread[0x16ff9f000]	time[1700360833335339000]
    37	thread[0x17062f000]	time[1700360833335300000]
    38	thread[0x1703ff000]	time[1700360833335266000]
    39	thread[0x1700b7000]	time[1700360833335222000]
    40	thread[0x16ff13000]	time[1700360833335361000]
    41	thread[0x1701cf000]	time[1700360833335266000]
    42	thread[0x1706bb000]	time[1700360833335332000]
    43	thread[0x16ff9f000]	time[1700360833335381000]
    44	thread[0x16fe87000]	time[1700360833335214000]
    45	thread[0x170143000]	time[1700360833335390000]
    46	thread[0x16ff13000]	time[1700360833335397000]
    47	thread[0x17002b000]	time[1700360833335342000]
    48	thread[0x1705a3000]	time[1700360833335278000]
    49	thread[0x170373000]	time[1700360833335152000]
    50	thread[0x17025b000]	time[1700360833335392000]
```

<br/>

### 线程TSD(`私有数据`)
- 再学习线程的私有数据前, 先来谈一下 <font color=deeppink>堆栈</font>. 在以前的学习中, 堆只是针对进程来说, 在Unix下进程的物理空间是隔离的(<font color=green>虚拟内存技术</font>), 每个进程的内存有一部分是动态扩展的, 这部分的内存就是所谓的堆. 用户可以通过标准库函数`malloc`来为当前进程扩展内存, 当然库函数底层必然调用系统调用(`如sbrk`). 后来出现的多线程在逻辑上被划分到了1个进程中, 用户在不同线程中调用malloc函数实际上还是在扩展进程的空间, 但我们常常又听见 <font color=deeppink>线程的堆栈</font>, 所以线程的堆其实是和进程有区别的. 从定义上来说, 线程的堆应该只属于线程所有, 其他的线程不能访问到. 从这个概念出发, 就有了线程的私有数据, 它其实就是线程的堆. 它的申请和释放并不是简单的库函数, 稍后会详细学习

- 线程的私有数据也称为 <font color=deeppink>线程的特定数据</font>(`thread-specific data`). 这部分的数据是线程独有的. 编译器常常用的`_Thread_local(C) thread_local(c++)`关键字实际上是编译器在编译时期调用了相关接口. 它是存储和查询某个线程相关联数据的一种机制.

> 在实际开发中, 线程必定存在自己的数据, 从逻辑上来看,这些数据是自己要维护的, 其他的线程不应该访问到. 所以实现这种机制 1. 利用线程ID的唯一性(<font color=green>进程中</font>)做数组映射, 但不现实(<font color=green>不连续, 不同平台实现不同</font>). 2. 操作系统提供其他的支持. 因为pthread是线程库, 所以这种机制由库提供(如glibc)
>
> <br/>
> 
> 基于前面的学习, 基本上可以肯定, TSD一定关联在TCB中.
<br/>

- 使用1, 创建全局唯一的key
```cpp
#include<pthread.h>
int pthread_key_create(pthread_key_t* key, void(* destructor)(void* key));
...
> 该函数由线程库(<font color=green>如glibc</font>)提供, 创建一个全局唯一的key. 要保证即使是多线程调用key也必须只初始化一次. 它的作用是其他线程通过该key存储自己的键值对. 通常配合`pthread_once`. 第2个参数则表示线程正常退出时被回调. 该回调函数的作用是释放申请的内存, 这一点在后面的设置键值对时介绍.

```cpp
#include<pthread.h>
pthread_once_t init_flag = PTHREDA_ONCE_INIT;
int pthread_once(pthread_once_t* initflag, void(*func)(void));
...
void* pthread_getspecific(pthread_key_t key);       // 注意这里不是指针
// 成功返回当时设置的内存
...
> 用户所存储的`value`一般是`malloc`所申请的内存(<font color=green>当然也可以是sbrk</font>). 它可以在`pthread_key_create`所提供的析构函数释放
...
int main(int args, char** argv){
    for(int i = -1; ++i < 5;){
        std::thread([]{
            pthread_once(&once, []{
                pthread_key_create(&key, [](void* val)->void{
                    fprintf(stdout, "destructor thread[%ld]\tval[%p]\n", pthread_self(), val);
                    // 这里可以释放内存
                });
            });

            sleep(1);
            auto val = malloc(sizeof(int));
            fprintf(stdout, "malloc thread[%ld]\tval[%p]\n", pthread_self(), val);
...
> 上述程序(`Linux`)只注册了一次`ptherad_key_create`, 但不同线程分别设置了自己的特定数据, 在线程结束时, 都回调到了析构函数

<br/>

```shell
malloc thread[281472851587520]	val[0xffff70000b60]
destructor thread[281472851587520]	val[0xffff70000b60]
malloc thread[281472826409408]	val[0xffff60000b60]
destructor thread[281472826409408]	val[0xffff60000b60]
malloc thread[281472834802112]	val[0xffff78000b60]
destructor thread[281472834802112]	val[0xffff78000b60]
malloc thread[281472843194816]	val[0xffff58000b60]
destructor thread[281472843194816]	val[0xffff58000b60]
malloc thread[281472818016704]	val[0xffff68000b60]
destructor thread[281472818016704]	val[0xffff68000b60]

## 每个线程都会回调, 并且将当时malloc的内存传递到析构函数中
```
...
> 该函数不会触发析构函数的回调, 只要调用1次, 所有的线程全部解除关联
...
int main(int args, char** argv){
    for(int i = -1; ++i < 10;){
        std::thread([]{
            pthread_once(&once, []{
                pthread_key_create(&key, [](void* val)->void{
                    fprintf(stdout, "destructor thread[%p]\tval[%p]\n", pthread_self(), val);
                });
            });
            ...
            auto val = malloc(sizeof(int));
            fprintf(stdout, "malloc thread[%p]\tval[%p]\n", pthread_self(), val);
            pthread_setspecific(key, val);
        }).detach();
    }
    pause();
...
> 上述程序(`MacOS`)只要某个线程调用了一次`pthread_key_delete`, 其他的线程也将解除关联. 

<br/>

### 书籍上的getenv实现
- 在学习书籍上getenv的实现时, 下面先看一下作者的实现
...
static char* _Nullable my_getenv(const char*  _Nonnull name){
    ...
    void* buf = pthread_getspecific(key);           // __code_get
    do{
        if(buf){
            memset(buf, 0, VAL_MAX_LEN);
            int i = -1;
            extern char** environ;
            while(!flag.test_and_set()){            // __code_lock
                auto len = strlen(name);
                while(*(environ + ++i)){
                    ...
                    这里遍历环境变量, 比较 name是不是 keyN, 然后取出对应的valN
#endif
                    if(strncmp(name, *(environ + i), len) == 0 &&
                       *(*(environ + i) + len) == '='){
                        strncpy(static_cast<char*>(buf), *(environ + i) + len + 1, VAL_MAX_LEN - 1);
                    }
                }
            }
            flag.clear();                           // __code_unlock
            break;
        }
        
        buf = malloc(VAL_MAX_LEN);                  // __code_malloc
        pthread_setspecific(key, buf);              // __code_set
        continue;
    }while(1);
...
> 这里并未像书籍上一样, 用一把锁锁住所谓的临界区, 需要说明的是: <font color=red>malloc, set, get没有被锁住是因为本身malloc就是线程安全的; 并且key所关联的内存对每个线程来说是独立的, 库函数在实现get/setspecific时内部是线程安全的, 所以没有必要加锁, 这里加锁的目的是因为`逻辑上还有一个my_setenv`函数, 它会改变全局变量environ的值, 所以若要实现必须保证environ的修改是互斥的. 本人使用的是CAS, 并没有用mutex</font>
>
> <br/>
>
> 书籍上的实现个人感觉加锁的粒度太大, 若在开发中用户不会调用`setenv或putenv`, 则整个`getenv`的实现是不需要加锁的. 锁的目的是互斥environ这个全局变量

<br/>

### TSD的析构函数调用
- 只有线程调用`pthread_setspecific`所传入的value不为空时, 线程正常终止时析构才会被调用
...
int main(int args, char** argv){
    ...
    std::thread([]{
        pthread_setspecific(key, nullptr);  // 值为空, 相当于没有注册
    }).detach();
    pause();
    return 0;
}
...
> 上述程序的析构并不会调用, 因为在整个过程中没有任何一个线程调用`pthread_setspecific`注册过键值对
...
### 线程变量
- 线程变量的具体概念就不详谈了. 这里简单描述一下: <font color=deeppink>所谓的线程变量就是变量作用域(<font color=green>可见范围</font>)只在当前线程可见. 其他线程不能可见</font>. 在C环境下和CPP环境下定义的格式不同
...
> 线程变量的实现原理必须借助操作系统完成. 从理论上来看:线程是动态的结构, 所以对应线程变量整个生命周期也应是在程序运行过程中诞生和释放. 对于操作系统来说线程的结构TCB 在内核实现时大小一定是固定的. 在TCB中应当有一份特定的区域(`理论上应该是大小固定的指针`)来关联所谓的所有的线程变量(`另外开辟的区域`. 当用`_Thread_local`明确定义出线程变量时, 编译器能确定工程中所有这种线程变量的大小, 所以编译器能够做到的是在编译代码成二进制程序时, 它能告诉操作系统在创建TCB时, 直接开辟多大的大小空间(`后续新的线程也是一样的`)来关联所有的`_Thread_local`变量. 而程序在运行过程中调用`pthread_setspecific`就相当于线程在开辟堆, 当然这种方式也会被关联到TCB中,这个工作应该由库来完成(`如glibc`),内存的释放就要由程序员来完成了.
> 然而Linux和MacOS在实现线程变量是不一样的:
>
> <br/>
>
> 在Linux中`_Thread_local`变量被称为`TLS(线程本地数据)`. 相对于用户调用的`pthread_setspecific`来说它更`"静态"`.其实现过程大致是: 内核在创建进程(`Linux中进程和线程不分家`)时, 直接开辟了所有线程变量的内存, 并在TCB中关联了内存的起点. 因为编译器在编译时就可以确定所有`__thread`变量内存, 那怎么将这个大小保留到运行时呢? 这里笔者没有深究, 但猜想有2种方案实现: 1. GNU操作系统环境下编译器链接调用CRT(`C运行环境`)时, 将大小传递给了CRT, CRT立即开辟空间然后关联到TCB相关的TLS字段中, 这个内存释放将由CRT来处理. 2. 内核直接在创建TCB时就直接开辟这个空间, 这个时间相对于第1种来说更早, 那就要求编译器生成的二进制程序文件中就记录了所有`__thread`变量的大小, 内核读取二进制文件时能获取到这个值, 这个内存要内核来管理.
>
> <br/>
>
> 但在MacOS中, 其实现就太直接了, `__thread`变量直接由编译器调用`pthread_setspecific`函数来处理. 
>
> <br/>
>
> 不管2种系统怎么实现, 用户使用时效果是一样的. Linux的实现从逻辑上更高效. 因为所有对`__thread`变量的操作都是直接寻址. MacOS则是借助`pthread_setspecific`, 有函数调用和执行开销
> 
> <br/>
> 
> 要怎么测试出来呢? 这里假设编译器会调用`pthread_setspecific`函数, 所以最直接的方案是在该函数打断点. 首先在MacOS环境下(<font color=green>分别测试C和C++环境</font>)
...
int main(int args, char** argv){
    a = 2;                  # 若在代码中不对a作操作, 则项目中没有地方用到a导致编译器会优化掉线程变量a
    pause();
    return 0;
}
```
...
> 这里通过脚本判断文件类型(<font color=green>C或C++</font>)
...
case "$suffix" in
    c)
        gcc ${filename} -lpthread -glldb -std=c11 -O3 -o ${filename}-c.out
        ;;
    cpp)
        g++ ${filename} -lpthread -glldb -std=c++17 -O3 -o ${filename}-c.out
        ;;
    *)
        echo "The file has an unknown extension."
        ;;
esac


## 注意这里虽然是在MacOS平台上, 但使用的编译器是gcc或g++, 并未使用clang\clang++, 原因是在2平台下使用同一编译做测试
```
...
./com.sh main.c         # 若是c++, 则 ./com.sh main.cpp

lldb main.c-c.out       # 调试
b pthread_setspecific   # 在该函数下打断点
r                       # 运行


# 断点停留在指定的函数
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
    frame #0: 0x00007ff811b145c6 libdyld.dylib`dyld4::LibSystemHelpers::pthread_setspecific(unsigned long, void const*) const
libdyld.dylib`dyld4::LibSystemHelpers::pthread_setspecific:
->  0x7ff811b145c6 <+0>: pushq  %rbp
    0x7ff811b145c7 <+1>: movq   %rsp, %rbp
    0x7ff811b145ca <+4>: movq   %rsi, %rdi
    0x7ff811b145cd <+7>: movq   %rdx, %rsi
Target 0: (main.c-c.out) stopped.
...
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
  * frame #0: 0x00007ff811b145c6 libdyld.dylib`dyld4::LibSystemHelpers::pthread_setspecific(unsigned long, void const*) const
    frame #1: 0x00000002000224b2 dyld`dyld4::RuntimeState::_instantiateTLVs(unsigned long) + 222
    frame #2: 0x00007ff811b04555 libdyld.dylib`tlv_get_addr + 296               # __shell_0
    frame #3: 0x0000000100003f60 main.c-c.out`main(args=<unavailable>, argv=<unavailable>) at main.c:32:5 [opt]
    frame #4: 0x00000002000163a6 dyld`start + 1942
```

<br/>

> PS: 需要说明的是本人的配置是<font color = red>M2架构</font>(`arm64`), 但在shell调试中的汇编指令却是`x86`, 原因是当前笔者所使用的终端(`iterm`)配置了Rosetta.
> `__shell_0`可以说明一下: 这里所调用的`tlv_get_addr`目的是获取`TCB中线程变量的起始地址`, 若在执行过程中观察函数, 其汇编使用的是`gs寄存器`, 该寄存器记录了TCB的地址. 这里不具体说了, 因为后面arm下的Linux对比下使用的是`tpidr_el0`, 原理是一样的
> 若是C++是一样的过程. 说明在MacOS环境下, 线程变量的本质就是编译器在运行时将`_Thread_Local`所修饰的变量转变成相关的接口调用. 
> 接下来看Linux环境下
...
```shell
gcc -O3 -lpthread -ggdb main.c -o main          # 编译

gdb main                                        # 开始调试
(gdb)b 7                                        # 设置断点 __code_0
(gdb)r                                          # 运行
Starting program: /tmp/lb/main
[Thread debugging using libthread_db enabled]
Using host libthread_db library "/lib/aarch64-linux-gnu/libthread_db.so.1".

Breakpoint 1, main (args=1, argv=0xfffffffffb08) at main.c:7        # 程序断点在 第7行(__code_0)
7		__builtin_printf("%p\n", &a);
(gdb) disassemble
Dump of assembler code for function main:
   0x0000aaaaaaaa085c <+0>:	stp	x29, x30, [sp, #-32]!               //     __shell_0
   0x0000aaaaaaaa0860 <+4>:	mov	x29, sp                             //     __shell_1
   0x0000aaaaaaaa0864 <+8>:	str	w0, [sp, #28]                       //     __shell_2
   0x0000aaaaaaaa0868 <+12>:	str	x1, [sp, #16]                   //     __shell_3        __cur
=> 0x0000aaaaaaaa086c <+16>:	mrs	x0, tpidr_el0                   //     __shell_4        
   0x0000aaaaaaaa0870 <+20>:	add	x0, x0, #0x0, lsl #12           //     __shell_5
   0x0000aaaaaaaa0874 <+24>:	add	x0, x0, #0x10                   //     __shell_6
   0x0000aaaaaaaa0878 <+28>:	mov	x1, x0                          //     __shell_7
   0x0000aaaaaaaa087c <+32>:	adrp	x0, 0xaaaaaaaa0000          //     __shell_8
   0x0000aaaaaaaa0880 <+36>:	add	x0, x0, #0x948                  //     __shell_9
   0x0000aaaaaaaa0884 <+40>:	bl	0xaaaaaaaa0740 <printf@plt>     //     __shell_10
   0x0000aaaaaaaa0888 <+44>:	adrp	x0, 0xaaaaaaaa0000          //     __shell_11
   0x0000aaaaaaaa088c <+48>:	add	x0, x0, #0x950                  //     __shell_12
   0x0000aaaaaaaa0890 <+52>:	bl	0xaaaaaaaa0730 <puts@plt>       //     __shell_13
   0x0000aaaaaaaa0894 <+56>:	bl	0xaaaaaaaa06e0 <pause@plt>      //     __shell_14
   0x0000aaaaaaaa0898 <+60>:	mov	w0, #0x0                   	    //     __shell_15       
   0x0000aaaaaaaa089c <+64>:	ldp	x29, x30, [sp], #32
   0x0000aaaaaaaa08a0 <+68>:	ret
End of assembler dump.
(gdb) info registers                                                //     __shell_16
x0             0x1                 1                                //     __shell_17
x1             0xfffffffffb08      281474976709384
x2             0xfffffffffb18      281474976709400
x3             0xaaaaaaaa085c      187649984432220
x4             0x0                 0
x5             0x6779b6b37f13283d  7456191540135929917
x6             0xfffff7f88b10      281474842004240
x7             0x410100000         17449353216
x8             0xffffffffffffffff  -1
x9             0x3fffffffffff      70368744177663
x10            0x0                 0
x11            0x0                 0
x12            0xfffff7e1ce48      281474840514120
x13            0x0                 0
x14            0x4                 4
x15            0x6fffff47          1879048007
x16            0x1                 1
x17            0xfffff7e39d28      281474840632616
x18            0x73516240          1934713408
x19            0xaaaaaaaa08a8      187649984432296
x20            0x0                 0
x21            0xaaaaaaaa0750      187649984431952
x22            0x0                 0
x23            0x0                 0
x24            0x0                 0
x25            0x0                 0
...
x27            0x0                 0
x28            0x0                 0
x29            0xfffffffff990      281474976709008
x30            0xfffff7e39e10      281474840632848
sp             0xfffffffff990      0xfffffffff990
pc             0xaaaaaaaa086c      0xaaaaaaaa086c <main+16>
...
(gdb) si
0x0000aaaaaaaa0870	7		__builtin_printf("%p\n", &a);           //     __shell_18
(gdb) p $x0                                                         //     __shell_19
$1 = 281474842453776                            
(gdb) x/20dw $1                                                     //     __shell_20
0xfffff7ff6710:	-134255008	65535	0	0                           //     __shell_21
0xfffff7ff6720:	0	0	-134708304	65535                           //     __shell_22
0xfffff7ff6730:	-134696344	65535	0	0
0xfffff7ff6740:	-135088968	65535	-135087432	65535
0xfffff7ff6750:	-135085128	65535	0	0
(gdb) c                                                             //     __shell_23
Continuing.                     
0xfffff7ff6720                                                      //     __shell_24

Breakpoint 2, main (args=1, argv=0xfffffffffb08) at main.c:8
8		__builtin_printf("hello\n");                                //     __shell_25(__code_1)
(gdb) c
Continuing.
hello

### 在linux下(arm64)并未看见明显的pthread_sepecific的跳转(虽然没有手动的去在该函数配置断点, 但实际上测试过程中发现并没有该函数的调用)
```

<br/>

> 以上在`Linux 5.15.0-76-generic`内核环境下, arm64的编译调试
> 
> <br/>
>
> `__shell_0 ~ __shell_25`是调试的过程
>
> <br/>
>
> `__shell_0`: 将寄存器`x29, x30`的值存储到`当前栈桢(main)中, 即 sp-32开始的位置`, 后面的叹号表示并同时将sp向低地址偏移32个字节. 说白了这一步的目的是建立main函数的栈桢. arm指令中约定`x29`寄存器(`也就是fp寄存器`)指向当前函数栈桢的的栈底, sp指向当前函数栈桢的栈顶. `x30`保存当前栈桢中上一函数的返回地址. 所以后续的`ldp	x29, x30, [sp], #32` 指令返回时, 还原了`x29, x30, sp寄存器(sp+32)`
>
> <br/>
>
> `__shell_1`: 保存寄存器`sp`的值存储到`x29`寄存器中. 经过上一步, sp已经减去了32个字节, 因为sp约定指向的是栈顶, 后续的入栈操作sp要不断向低地址偏移, 所以再用`x29`来记录此刻还未做任何入栈时的栈顶地址, 相对的当前函数中就一直指向栈底
> 
> <br/>
>
> `__shell_2`:将`w0`的值写入到局部变量中(`sp + 28`)
> 
> <br/>
>
> `__shell_3`:保存`x1`的值到局部变量中(`sp+16`). arm中`x0 ~ x7`一般用来传递函数的参数, 因为后续有2次printf的调用. 所以先保存它们的值到局部变量中. 至于后续未还原`x0, x1`是因为当前main函数并未再用到它们, 所以就没有还原
>
> <br/>
>
> `__shell_4`:是 <font color=red>当前函数断点的位置</font>(`__code_0`), 此刻准备调用printf, 并传递2个参数`字符串 "%p\n" 以及 &a`, 所以编译器要向`x0,x1`中写入要传递的参数值. 即后面的几步就是为调用`printf`作准备. <font color=red>需要注意: 此刻`__shell_4`的汇编并未执行, 后面的`__shell_16`是立即查看当前所有的寄存器值, 可以看到`x0`的值是`0x1(__shell_17)`</font>. 虽然不能看出编译器调用函数的约定, 但从后面的汇编可以看出, 还是从左向右传递. 这不同于`x86`约定. 所以`&a`应该由`x1`传递, `%p\n`应该由`x0`传递. <font color=red>当前汇编的作用就是获取TCB中TLS的地址(`tpidr_el0`), 然后存储到`x0`中</font>, 所以当该条指令过后`x0`指向了`TLS`
>
> <br/>
>
> `__shell_5`: `x0 = x0 + (0x0 << 12)`, 当cpu执行到这一步后, x0的值其实并未改变, 所以还是指向`TLS`
>
> <br/>
>
> `__shell_6`: `x0 = x0 + 0x10`, 即`x0`偏移16个字节, 此刻真正指向`线程变量a`的地址. <font color=red>注意此刻指令还停留在`__shell_4`之前</font> 
>
> <br/>
>
> <font color=red>立即执行`__shell_18`查看完所有寄存器, `x0`的值(`__shell_19`)为`0x1`.</font>
> 
> <br/>
>
> 再执行`__shell_4`后根据前面所分析的`x0指向了TLS, 然后向后偏移16字节的地址就是线程变量a的地址`, 所以`__shell_18`这里, 再次查看此刻`x0`的值(`__shell_19(TLS的0xfffff7ff6710), __shell_20,__shell_21, __shell_22`) 发现`x0`后的第16字节刚好是`__shell_24对线程变量a地址的打印`
>
> <br/>
>
> PS: 综上Linux中对`__thread`变量的实现机制不同于`MacOS`, 它直接在TCB中建立了所有`__thread`变量的内存空间, 在运行过程中, 直接通过`mrs	x0, tpidr_el0`指令获取到`TLS`的地址后, 再做相应的偏移(`add	x0, x0, #xxx`). 从这一点来看, 效率上的确比`MacOS`要快. 至于Linux是怎么实现的, 笔者在上面所作的猜想: <font color=red>可能是编译器在链接CRT时, CRT运行库开辟了空间. 也可能是内核在读取二进制程序文件时, 直接在创建TCB时就创建好大小. 所以笔者才说`__thread`变量相对`pthread_setspecific`更静态</font>
>
> <br/>
>
> PS: 关于其他指令这里也简单提一下:
...
> <br/>
>
> `__shell_7`: `x1 = x0`. 因为约定`x1`作为printf的第2个参数`&a`,所以它指向了`此刻线程变量a的地址`.
>
> <br/>
>
> `__shell_8, __shell_9`: 寻址到`%p\n`的地址, 它是一个常量字符串, 保存在常量区, 编译器可以找到它的地址, 这2步就将`x0`指向了这个常量字符串, 作为第1个参数传递给`printf`
>
> <br/>
>
> `__shell_10`: 调用`printf`
>
> <br/>
>
> `__shell_11, __shell_12, __shell_13`: 已经调用完`printf`(`__code_0`),接下来第2次调用. 但编译器这里做了优化, 因为没有可变参数传递(`只传递了一个字符串"hello"`), 所以它优化成了`puts`调用. 所以`x0指向了常量"hello"`, 并调用`puts`
>
> <br/>
>
> `__shell_14`: 调用函数`pasue`, 使程序挂起
>
> <br/>
>
> `__shell_15`: 清零`w0`.
...
> <br/>
>
> PS: 上面严格意义上来说, 并不能确定 <font color=red>对线程变量a赋值时, 会不会调用`pthread_setspecific`, 是笔者故意留下来的. 事实上在源代码中加入`a = 2`经过汇编调试后, 结论还是一样,Linux直接寻址填充值, 并不会调用`pthread_setspecific`. 有兴趣的话自己可以测试看一下</font>
...
### 线程变量的内存区域
- 从上面的探究中, 可以得出在MacOS下线程变量是通过调用`pthread_setspecific`, 所以对应的线程变量的位置应该在`malloc`的堆区. 对应的Linux的`TLS`地址若是由内核实现, 则倾向于静态, 它的内存地址应该在较高区域(`Unix下C内存布局,见第7章`). 对于下面同样一份代码在2家平台下的测试效果也和预想的一样
...
int  main(int args, char** argv){
	__auto_type c = __builtin_malloc(sizeof(int));
	printf("global-a-addr:\t%p\nernno-addr:\t%p\nthread-b-addr:\t%p\nmalloc-c-addr:\t%p\n", &a, &errno ,&b, c);
	return 0;
}
```
...
- 现如今的errno已经被实现为了线程变量机制[^ann-errno-1]). 所以不同线程所访问到的errno是隔离的. <font color=deeppink>为了测试errno是不是线程变量, 现在在Linux中作出测试</font>
...
```shell
## mac下
global-a-addr :0x104c2a01c
ernno-addr   :0x2050372ec
thread-b-addr:0x6000003c8060        ## __shell_0
malloc-c-addr:0x6000003c8040        ## __shell_1
command-arg  :0x30dcb4460
environ      :0x30dcb41a0



## linux下
global-a-addr :0xaaaac63f1014
ernno-addr   :0xffffa19d6738
thread-b-addr:0xffffa19d6720
malloc-c-addr:0xaaaad1b8a2a0
command-arg  :0xffffe973ad8d
environ      :0xffffe973aae8
```

<br/>

> 发现: 在Mac上用户定义的`__thread`变量确实会将其分配在堆中(`__shell_0, __shell_1`), 而对应在Linux下被定义在了接近`高地址的命令行参数的区域`. 从C内存布局来看, 在高地址(`这里指命令行,环境变量区域`)的内存一般是只读的, 但`__thread`变量的地址在这片区域又显得较低. 这就是操作系统的规定了, 可能这种线程变量可读可写, 所以不能在命令行区域, Linux对C程序的划分有更多的细节.

<br/>

### errno
- errno在标准规定下要求多线程访问时不能互相影响, 所以相应的实现机制就是线程变量. 通过前一个例子的测试, 在Linux平台下, 它的地址紧挨着用户定义的线程变量. 并且只错开了0x18个字节. 说明程序启动后, 不仅仅有`errno`系统的线程变量, 还有其他定义的同类变量. 
- 因为errno基本被定义成为宏, 以下面的例子来学习errno
...
> 上面的程序连main函数也没有, 但这里要测试的并不是链接后的程序, 因为ernno是一个宏, 所以可以通过预编译来看一下errno到底是什么东西(`Linux平台`)

<br/>

```shell
gcc -E main.c -o main.e     ## 直接预编译
...
> 可以发现, ernno实际上就是一个函数调用.  因为要求它是线程变量, 可以想到函数内部应该是一个`__thread xxx`变量, 然后返回了它的地址. 
>
> PS: <font color=red>之前猜想过, Linux的`TLS`所指向的内存可能有2种开辟方案: 一种是CRT, 另一种是内核. 通过上一小节测试样例可以基本确定, 应该是内核在加载二进制程序时就直接在TCB中创建好了, 因为errno变量地址和用户定义的线程变量的地址基本在一起, 它们的地址在内存布局中的高地址区域, 安排必须是内核在加载程序文件时, 就做好的</font>
...
### 线程与信号
- 第10章的信号学习为了方便, 特意脱离的线程. 但即使这样, 在进程环境下的信号处理也是十分复杂的. 但从操作系统的实现来看, 线程和进程有着密切的关联, 脱离线程来谈信号从逻辑上来说是错误的, 所以附加线程来学习信号会更加复杂

- 以书籍为例: 每个线程都有自己的信号屏蔽字(`mask-pending`), 但是信号处理是进程中所有线程共享的. 所以可以有以下结论:
    1. 单个线程可以阻止某个信号(<font color=green>阻止某个信号不会修改信号处理</font>)
    2. 只要有线程修改了某个信号的处理行为, 则会影响到整个进程下所有的线程
    3. 某个线程忽略了某个信号(`相当于修改了信号的处理`), 其他线程可以自己恢复

- <a id=signal-thread>信号是异步[^ann-async]的, 信号未决前当前线程是挂起的, 当信号由内核制造出来时会随机派发到某个线程. 但有些硬件故障的信号并不是随机的, 会定位到产生这个异常的线程中去


<br/>

- <font size = 5>测试1--信号处理程序是共享的</font>
```cpp
...
static inline void cout_common(const char* info = "main"){
	cout << "\nhand\t[\t" << pthread_self() << "\t]\n";
	cout << info << " define handle\n";
	cout << "change?(1:main 2:sub)\n";
}
...
> 上述程序启动后由主线程([^ann-main-thread])率先注册处理函数.  然后创建线程, 2者同时进入死循环, 查看change的值. 当用户键入`Ctrl + C`后引发`SIGINT`信号, 进程在处理程序中打印当前线程标识. 然后等待用户输入值来切换线程处理函数. 整个测试过程如下:

<br/>

```shell
./main
main:	[	281473118531296	]
thread	[	281473113735616	]           # 程序启动后 主线程和子线程的id
^C                                      # 此刻2条线程已进入了死循环, 这里键入了 Ctrl + c, 注意此刻状态下进程的处理函数是 main
hand	[	281473118531296	]           # 进程收到 SIGINT信号, 进入信号处理,输出当前线程(主), 也就是说内核将信号派发给了主线程 
main define handle                      # 打印标识是main
change?(1:main 2:sub)                   # 
2                                       # 在信号处理函数中等待用户确认要不要切换, 用户输入2表示切换信号处理(sub), 这一步后子线程循环中会设置信号处理为sub
^C                                      # 在键入 Ctrl+c前, 2条线程还是在无限循环中, 该步产生SIGINT信号
hand	[	281473118531296	]           # 这里是细节, 注意响应的线程还是主线程, 但打印的标识是sub
sub define handle
change?(1:main 2:sub)                   # 信号处理切回main
1
^C                                      # 产生SIGINT
hand	[	281473118531296	]           # 信号还是被主线程处理, 并且打印标识是main
main define handle
change?(1:main 2:sub)
^\zsh: quit (core dumped)  ./main       # 强制退出
```

> 测试的结果是线程的处理程序是公用的, 任何一个线程若是改变了信号的处理后, 其他线程也会受影响, 但<font color=red>不能体现出信号被随机调用, 来回2次的信号处理都是主线程</font>, 为了测试出随机性, 看下面的测试2

<br>


- <font size = 5>测试2--信号被随机派发</font>
```cpp
#include<unistd.h>
#include<sys/signal.h>
#include<iostream>
...
static inline void cout_thread(const char* info = "main"){
	printf("%s\t[\t%p\t]\n", info, pthread_self());
}
...
> 上述程序分别在2个平台下进行测试, 测试模式有2种: 手动在shell中键入`ctrl + c`; 使用脚本发送`SIGINT`, 先说结果: <font color=red>基本都是主线程在处理, 内核将信号派发到子线程的概率较低</font>. 下面是4张动态图给出测试过程和结果

<br/>

> 首先是Linux下, 手动摁下`Ctrl + c`

---

<img src="assets/11_12_10.gif"/>

---

> 测试发现, 基本由主线程处理. 下面是通过命令行通过脚本发送信号

---

<img src="assets/11_12_11.gif"/>

---

> 可以看到, 最开始第1次的时候由子线程处理了一下. 但大部分情况还是主线程在处理, 下面以样的方式在Mac中测试. 首先通过手动模式

---

<img src="assets/11_12_12.gif"/>

---

> 同样的效果(<font color=green>事实上笔者测试过, 在Mac上信号由子线程处理的情况还是比较容易出现的</font>). 接下是通过脚本发送信号


---

<img src="assets/11_12_13.gif"/>

---

> 很明细Mac上的效果要比linux上更随机. 为了进一步测试出是不是真不只有主线程在调用, 下面的代码(`Linux平台`)在主线程中屏蔽了信号
...
> 上述程序做一样的测试. 结论是Linux内核基本会固定的选择一个子线程去处理信号, 如发送100次信号, linux会适当的选择第3个子线程来处理这100次的信号(`不会打印100次, 因为别的进程在连续发送信号时, 测试程序可能已经正在信号处理中, 会有信号丢失, 这一点在第10章已经详细学习过`). 
>
> Linux中出现这种效果的原因可能是: 响应信号的线程可能被缓存, 下次同样信号直接从缓存中取它, 当然该线程虽然在缓存中, 但也是挂起的. 这纯属个人猜测


### 信号只会挂起当前所在的线程
- 一直以来笔者始终认为当硬件中断发生到信号产生, 再到派发处理前, 进程中所有的线程都是挂起状态. 但这里经过测试发现只有当前线程会被挂起, 其他线程并不影响
...
> 上述程序有2个线程在运行时, 都将处于死循环. 当`SIGINT`发生时, 内核会将信号指定到进程中的某个线程执行, 当线程调用处理函数时, 直接进入了挂起状态. 通过测试可以发现, 当发生第1次信号时, 2条线程中只有处理信号的线程进入了挂起. 另1条还在占据cpu. 当第2次信号发生时, 内核只能将信号交给另1条线程, 接着该线程执行信号处理后, 也进入了挂起


---

<img src="assets/11_12_14.gif"/>
...
### 线程的信号掩码
- 在第10章信号的学习中, 简述了信号在进程中的处理过程. 系统会为当前进程维护信号屏蔽字(`mask-pending`), 而现在是多线程环境, 线程隶属于进程, CPU调度单位是线程, 即: 每条线程都会被它所在的CPU产生的中断打断, 进而被派发信号. 相应的线程的`mask-pending`应该是独立的. 事实上OS的实现也是如此. 
- 现代的OS实现里, 进程只是一个容器(`PCB`), 真正描述任务的结构是线程(`TCB`), 信号屏蔽字已经独立到每条线程中, 为了区分于`早期的进程信号`, 规定了线程层面的信号屏蔽字, 如下:

```cpp
#include<signal.h>
int pthread_sigmask(int how, sigset_t* set, sigset_t* o_set);
...
> 首先它的功能和`sigprocmask`一样, 但返回不一样, 这里是直接返回错误编号; 而`sigprocmask`函数作为系统调用返回的是-1, 并设置errno.
> 
> 书籍上描述`sigprocmask`在多线程的环境中是未定义的, 即标准没有规定明确的行为. 下面分别在Mac和Linux中对系统调用`sigprocmask`的行为作测试
...
#if 0
    代码中的  __code_signal_pause 的作用是 让OS在派发信号时, 能交给子线程处理.
        因为不管在Mac还是在Linux, 正常情况下内核基本会将产生的信号交给主线程来处理,
        交给子线程的概率较小, 所以加上pause后, 当线程进入到信号处理流程中时, 会直接挂起,
        这样再产生信号时, 因为是多线程, 内核会重新查找当前进程中的其他线程, 将信号交给
        他们处理(若只有一个线程, 则响应信号后因为pasue, 不会从信号处理中返回, 导致mask位
        一直为0, 后续产生SIGINT也是不会响应的)
#endif
```
...
> 对于上述代码, 必须测试4种情况
>   1. 主线程调用`sigprocmask`屏蔽
>   2. 主线程调用`pthread_sigmask`屏蔽
>   3. 子线程调用`sigprocmask`屏蔽
>   4. 子线程调用`pthread_sigmask`屏蔽
>
> <br/>
>
> 再加上2个平台, 所以共要测试8次. 代码中定义了3个宏
>   1. `SUB_THREAD`: 表示子线程屏蔽
>   2. `MAIN_THREAD`: 表示主线程屏蔽
>   3. `PROC`: 表示屏蔽的方式, 若定义了这个宏, 则调用`sigprocmask`
>
> 下面先在Mac上测试

```shell
### 编译生成4种程序
##### 1. SUB_THREAD.out     --> 子线程 调用 pthread_sigmask屏蔽
##### 2. MAIN_THREAD.out    --> 主线程 调用 pthread_sigmask屏蔽
##### 3. PROC-SUB_THREAD.out--> 子线程 调用 sigprocmask屏蔽
##### 4. PROC-MAIN_THREAD.out-->主线程 调用 sigprocmask屏蔽


proc=PROC                       # PROC宏
arr=(MAIN_THREAD SUB_THREAD)    # 其他2个宏

## 生成 SUB_THREAD.out 和 MAIN_THREAD.out
for i in {1..{#arr[@]}};clang++ -lpthread -std=c++20 -glldb -D${arr[i]} main.cpp -o ${arr[i]}.out

## 生成 PROC-SUB_THREAD.out 和 PROC-MAIN_THREAD.out
for i in {1..{#arr[@]}};clang++ -lpthread -std=c++20 -glldb -D${arr[i]} -D$proc main.cpp -o $proc-${arr[i]}.out


### 测试1 SUB_THREAD.out        
#### 推测结论: 因为是子线程去屏蔽自己的信号屏蔽字, 所以不会影响到主线程, 所以程序运行起来后
#### 摁下ctrl + c后,主线程会响应一次后, 挂起, 然后继续键入相同的按键, 程序不会有任何线程响应
/SUB_THREAD.out
pid	[	19740	]
main	[	0x2042e1240	]       ## 程序启动后, 主线程输出自己的 thread_id
thread	[	0x30c4b0000	]       ## 程序启动后, 创建子线程后输出子线程的tid, 紧接着子线程调用了 pthread_sigmask修改了自己独有的mask, 屏蔽了SIGINT信号
^C                              ## 用户在shell摁下ctrl + c后, 产生SIGINT信号
hand	[	0x2042e1240	]       ## 这里看到主线程处理了该信号,在信号处理中, 主线程会pause被挂起
^C^C^C^C                        ## 连续产生SIGINT后, 没有任何线程响应(主线程信号处理时被pause),子线程在启动后就屏蔽了SIGINT,所以不会响应




### 测试2 MAIN_THREAD.out
#### 推测结论: 大致过程和1一样, 但第1次响应信号的是子线程, 接着后续的SIGINT没有任何线程响应
/MAIN_THREAD.out
pid	[	19781	]
main	[	0x20125d240	]        ## 程序启动后, 主线程输出自己的 thread_id, 它直接调用 pthread_sigmask屏蔽了自己的mask位
thread	[	0x306294000	]        ## 程序启动后, 创建子线程后输出子线程的tid, 紧接着子线程pause
^C                               ## 第1次SIGINT
hand	[	0x306294000	]        ## 发现是子线程来响应这个信号, 因为主线程已经屏蔽了, 然后子线程会在信号处理中pause, 并永久不从信号处理中返回(mask为0)
^C^C^C^C^C                       ## 连续的SIGINT, 没有任何信号响应




### 测试3 PROC-SUB_THREAD.out
#### 推测结论: 子线程调用了 sigprocmask, 按书籍上所说, 这个操作应该是未定义的. 在Mac上线程模型和进程模型有上下级关系(和Linux不一样)
##### 所以这个操作后猜想Mac应该是屏蔽了进程中所有线程, 即所有的线程都不会响应SIGINT
...
### 测试4 PROC-MAIN_THREAD.out
./PROC-MAIN_THREAD.out
pid	[	20009	]
main	[	0x203104240	]
thread	[	0x309fab000	]
^C^C^C
...
######### 接下来在Linux平台上, 效果完全不一样
### 测试5 SUB_THREAD.out
./SUB_THREAD.out                
pid	[	10559	]
main	[	281473462824672	]
thread	[	281473458028992	]       
^C
hand	[	281473462824672	]
^C^C^C^C

## # 子线程启动后, 屏蔽了自己mask, 整个过程和Mac上一样的行为
...
./MAIN_THREAD.out
pid	[	10598	]
main	[	281473444904672	]
thread	[	281473440108992	]
^C
hand	[	281473440108992	]   # 子线程响应后永久挂起(不响应其他信号, 相当于stop)
^C^C^C^C

## 同样的道理, 主线程屏蔽了自己的mask, 整个过程没有逻辑上的毛病



### 测试7 PROC-SUB_THREAD.out
./PROC-SUB_THREAD.out
pid	[	10605	]
main	[	281473411682016	]
thread	[	281473406886336	]       # 子线程启动后调用 sigprocmask
^C
hand	[	281473411682016	]       
^C^C^C^C^C^C^C^C^C

### 这个行为居然和 SUB_THREAD.out一样




### 测试8 PROC-MAIN_THREAD.out
./PROC-MAIN_THREAD.out
pid	[	10608	]
main	[	281473261833952	]
thread	[	281473257038272	]
^C
hand	[	281473257038272	]
^C^C^C

### 这个行为和 MAIN_THREAD.out 一样
```

<br/>

> 结论: 在Linux上`sigprocmask`和`pthread_sigmask`行为是一样的, 因为Linux中的线程和进程其实是同一种东西, 他们的TCB是独立的, 只不过进程的PCB(`其实在Linux上也是TCB`)和线程之间通过clone的指定有些空间是共享的, 就实现了进程和线程的机制. 在Mac中就不一样了, 可能Mac中的进程和线程是严格区分开的, 在响应信号时Mac可能先查看进程的PCB中的信号屏蔽, 再看当前线程下的TCB的信号屏蔽


<br/>

- 在第10章中`sigprocmask`函数会在返回前检查有没有未决信号, 有的话会调用一次. 具体见`第10章 sigprocmask返回前可能递送信号`. 可以推测出: `pthread_sigmask`也有同样的行为, 至少在Linux上. 下面通过以下代码来在2平台上测试一下
...
static inline void print_pending(){
	sigset_t set;
	sigpeding(&set);
    // linux中的sigset_t长度是128 * 8 = 1024字节的大小, 但前32位是标准的信号
    // 所以在linux平台上, 这里直接转成 uint32_t* 只读取前32字节
	cout << bitset<sizeof(uint32_t) * 8>(*((uint32_t*)(&set))) << endl;;
}
...
static inline void cout_thread(const char* info = "main"){
    printf("%s\t[\t"
            "%ld"           
            "\t]\n", info, pthread_self());
    // 若是在Mac平台, 这里 %ld 要换成 %p, 因为Mac的 pthread_t是指针
}
...
int main(int, char**){
    ...
    // 1. 屏蔽SIGINT
    // 2. 打印pending位 
    /////// 因为此刻没有信号产生, 所以pending为0
    /////// 注意前面屏蔽了SIGINT, 所以内核置SIGINT的mask位为0
    // 3. 自己发送信号(kill), 导致pending为1, 产生未决信号, 但因为 mask & peding == 0, 不处理
    // 4. 打印查看, 此时 pending 为1
    // 5. 还原当前线程的信号屏蔽
    /////// 此刻由于有未决信号, 函数在返回后进入到内核睡眠.
    /////// 下一次调度时, mask & pending == 1 会调用一次信号处理
    /////// 在以前学习sigprocmask时, 笔者认为应该是sigprocmask函数在返回后, 再进入睡眠
    /////// 等待下一次调度, 然后再调用信号处理, 现在想想应该是函数内部发现有未决信号后
    /////// 先强制睡眠, 醒来后因为未决调用信号处理后, 再返回到sigprocmask, 函数再返回到用户
	static auto sig_mask = []{
		sigset_t s,os;
		sigemptyset(&s);
		sigaddset(&s,SIGINT);
		pthread_sigmask(SIG_BLOCK, &s, &os);        // 1

		print_pending();                            // 2

		kill(getpid(), SIGINT);                     // 3
        // pthread_kill(pthread_self(), SIGINT)     // if Mac

		print_pending();                            // 4

		pthread_sigmask(SIG_SETMASK, &os, nullptr); // 5
		print_pending();
	};
...
> 本程序中调用的是`sigpending`, 书籍中并没有提到线程获取自己屏蔽字的接口, 所以暂且认为该函数获取到的是线程自己的pending. 测试结果如下:

```shell
### 子线程去测试(Linux)
g++ -DSUB_THREAD -ptherad -std=c++17 main.cpp -o SUB_THREAD.out
./SUB_THREAD.out
pid	[	10843	]
main	[	281473534738144	]
thread	[	281473529942464	]
00000000000000000000000000000000        # kill前, 发现第2位(SIGINT对应的序号位)为0, 表示没有未决
00000000000000000000000000000010        # kill后, 有未信号

hand	[	281473529942464	]           # 信号处理被调用, 因为还原信号时, 在pthread_sigmask返回前发现有未决, 所以会有1次自动调用
00000000000000000000000000000000        # 调用完毕后, peding被清零
sub over
main over

### 主线程去测试(Linux)
g++  -ptherad -std=c++17 main.cpp -o MAIN_THREAD.out
./MAIN_THREAD.out
pid	[	10858	]
main	[	281473207750368	]
00000000000000000000000000000000
00000000000000000000000000000010

hand	[	281473207750368	]
00000000000000000000000000000000
main over
...
> 以上程序在Mac中测试时, 注意修改一下细节(`kill改为pthread_kill`), 过程和Linux一样
...
###  线程阻塞
- 在信号的学习中, 当进程调用`pause`后, 自身会挂起, 在阻塞挂起期间若产生了信号则可能被打断进入信号处理(<font color=green>前提是该信号建立了处理</font>). 这其实就是典型的 <font color=deeppink>同步</font>, 即线程能主动的在任何位置等待某个特定的事件发生. 但这种操作不能等待指定的信号. 我们可以自己简单来实现等待特定信号: 如要等待`SIGINT`, 先屏蔽除开该信号的所有信号, 然后调用pause. pause返回后再还原信号. 但这样做有2个问题: 1. 其他信号被无辜屏蔽; 2. 即使进程中不发生其他信号, 但在阻塞这些信号后到pause真正被执行前, 有1个时间窗口, 若真的发生了信号则可能直接终结进程. 其中对于第2个问题, POSIX提供了`sigsuspend`, 它能将屏蔽信号和建立pause这2步原子化. 但同样还存在第1点. 在多线程中POSIX规定了新的接口来为线程单独阻塞某个信号. 
```cpp
#include<signal.h>
...
> 参数set: 指定要等待的信号集
>
> 参数signop: 是一个整型指针, 表示被哪个信号所唤醒
>
> 函数的作用是阻塞等待指定的信号(`可能多个`). 只要指定的信号产生, 就立即返回

- 函数使用上有一些细节. 在使用前要屏蔽指定的信号, 然后该信号会由sigwait来解除阻塞并直接挂起, 这个过程是原子的. 若不屏蔽则在调用sigwait前可能要等待的信号刚好发生, 而没有真正由sigwait所唤醒. 当后续该信号将不再产生, 则线程将永久挂起(`假如没有其他信号产生`). 下面用sigwait来实现第10章节信号驱动(`信号阻塞与暂停的原子操作`)
...
static inline void print_line_star(void){
	for(int i = -1; ++i < 5;){
		cerr << "*";
		sleep(1);
	}
...
static inline void print_exclamaiton(void){
    cout << "!" << flush;
}

static inline void sig_hand(int signo){
    print_exclamaiton();
}
...
int main(int arg, char** argv){
	...
	int n = 0;
	while(1){
		pthread_sigmask(SIG_BLOCK, &s, nullptr);        // 自己先阻塞,保证此之后调用sigwait前不发生该信号的处理

        if(sigwait(&s, &n)){                            // 等待SIGINT
            cerr << "sigwait: " << strerror()
            exit(-1);
        }

		if(n == SIGINT)                                 // 若是SIGINT则打印!
            print_exclamaiton();
        else
            continue;                                   // 这里基本不会出现

		print_line_star();                              // 打印一行 *
	}
	return 0;
}
```
...
> 该程序的效果和最终的信号驱动程序一样. 同时也说明了sigwait也会调用未决信号. 因为在一行的输出中若多次键入`Ctrl + c`则当前行打印结束后,会紧接着响应1次信号, 并打印下一行(`sigwait直接返回了`)

<br/>

### sigwait和原信号处理函数
- sigwait函数一定会解除等待信号的阻塞, 那么当用户已经为等待信号配置了信号处理函数, 那么等待期间发生了该信号后, 是调用信号处理还是直接从sigwait返回呢? 下面写个案例测试一下:
...
int main(int arg, char** argv){
	signal(SIGINT, sig_hand);           // __code_register_hand
	sigset_t s;
	sigemptyset(&s);
	sigaddset(&s, SIGINT);
	int n = 0;
	do{
		sigwait(&s, &n);                // __code_wait
		if(n == SIGINT)
			cout << "wake" << flush;    // __cout_wake

        while(pause());                 // __code_pause
	}while(0);
	return 0;
}
```
...
> 程序开始就为SIGINT注册了信号处理函数(`__code_register_hand`). 当程序挂起时(`__code_wait`), 尝试发出SIGINT信号, 测试结果是打印wake(`__cout_wake`). 而当程序再次被挂起时(`__code_pause`), 再次发出SIGINT后, 调用的却是信号处理. 说明sigwait直接屏蔽了原来的信号处理. 原理是内部修改了信号处理, 在返回后又改回了原来的处理. 测试结果如下:

<br/>

```shell
./main
^Cwake^Chand                            // 第1次发出SIGINT, 打印wake(__cout_wake)
^Chand                                  // sigwait返回后, 再次发出SIGINT, 发现调用的是原来的信号处理    
^Chand
^Chand
^Chand
^\zsh: quit (core dumped)  ./main
```

<br/>

### sigwait会修改信号处理函数
- 这里因为sigwait不是系统调用, 可以直接看glibc的源码
...
// 用户调用sigwait时, 来到这里,glibc在选择编译选项时, 这里应该是inline函数(函数并不复杂)
...
#if 0
    先简述一下原理: sigwait利用的是sigsuspend实现的, sigsuspend的作用在第10章详细学习过.
    它的参数也是一个信号集, 信号集中存在的信号不能唤醒sigsuspend. 其他信号可以. 
    所以tmp_mask的作用是就是 删除掉set中存在的信号, 然后当作参数传递给sigsuspend. 
    这样sigsuspend就能被set中的信号唤醒. 
#endif

    /**
        定义所有的可能指定的 sigaction
    */
  struct sigaction saved[NSIG];     // NSIG为64, 但事实上已经包含了31个标准信号
  
  int save_errno;
  int this;

  /**
    这里action的作用是: 参数set中由用户所指定的信号(唤醒sigsuspend)统一公用的信号处理函数.
    这里也说明了一个结论: sigwait会屏蔽用户以前的信号处理, 因为目的是返回到用户调用的地方,
    所以必须禁用以前的信号处理
  */
  ...
  was_sig = -1;
    ...
    /*
        遍历所有的信号, 若发现set中指定了, 则从 tmp_mask中移除, 移除的这些信号就是唤醒sigsuspend的信号
    */
  for (this = 1; this < NSIG; ++this)
    ...
    // 改变原信号的信号处理, 并同时保存到 save[this], 用于唤醒后的还原
    ...
    // 此时的 tmp_mask已经移除了所有set中指定的信号(可以被这些信号唤醒)
...
/*
    内部公共的处理函数. 这里很巧妙. 
    因为sigwait的需要填充被唤醒的信号, 所以
    该函数的作用就是发生信号时, 记录下这个信号

    但这里用了一个全局变量, 笔者没有认真分析会不会出问题
    因为多线程下都可能调用 sigsuspend, 每条线程以不同的
    信号唤醒时, 大家用同一个全局变量来记录. 所以...
    这个问题留下后面的小节来说明
    */
static void ignore_signal (int sig){
  was_sig = sig;
...
### sigsuspend在多线程下
- 前面小节留下了一个问题: <font color=red>glibc中使用全局变量记录信号处理有没有问题?</font> 我们心里知道一定没有问题, 因为glibc不可能出这种低级错误!! 但原因是什么呢? 现在假设它是成立的, 则满足的条件必须是: sigsuspend只能被某个线程唤醒. 因为只有它成立了, 公用的处理函数就只能被某1个线程独占调用, 所以这样就没有问题. 其他线程在它返回后, 其实还处于挂起状态. 所以了验证这个猜想, 来写一个程序测试`sigsuspend`在多线程下的情况
...
static vector<pthread_t> tids;

atomic_flag flag = ATOMIC_FLAG_INIT;    // CAS, 用来保护全局变量vector

static inline void print_thread(const char* info = "main"){
    printf("%s:\t[\t%lu\t]\n", info, (uint64_t)pthread_self());
}

#define NUM (10)                        // 10条子线程

int main(int args, char** argv){
    print_thread();
    signal(SIGINT, [](int no){
        print_thread("sub hand over");
    });
    sigset_t s, os;
    sigemptyset(&os);
    sigemptyset(&s);
    sigaddset(&s, SIGINT);
    pthread_sigmask(SIG_BLOCK, &s, &os);
    ...
    for(int i = -1; ++i < NUM;)
        thread([]{
            ...
            print_thread("sub new");
            tids.push_back(pthread_self());     // 线程安全, 将自己的tid加入到全局vector中
            flag.clear();
            sigset_t s;
            sigemptyset(&s);
            sigsuspend(&s);
    ...
    // 开始向每个线程发送信号, 这里用随机数, 每次从当前vector中随机出某条线程, 向它发送信号, 然后
    // 从vector中删除它.
    while(tids.size() && cin >> args){
        default_random_engine e{random_device{}()};
        auto r = uniform_int_distribution<int>(0,(int)tids.size() - 1);
        auto idx = r(e);
        auto tid = tids[idx];
        fprintf(stderr, "%d-->%ld\n", idx, tid);
        tids.erase(tids.begin() + idx);
...
> 上述程序会在主线程中阻塞`SIGINT`. 子线程创建后将自己的`pthread_self()`加入到全局数组中, 这个过程是互斥的. 只有等到所有线程加入到数组中后, 主线程才开始从标准输入读取数据, 读取1次对线程发送信号, 直到10个线程发送完毕, 自己再解除信号阻塞, 并睡眠. 下面是测试结果

```shell
echo {1..20} > a.txt                # "1 2 3 4 ... 20" 存入到文件
./main < a.txt                      # 调用main函数, 并重定向标准输入(从文件中读取20个数字), 源码中读取10个后就结束了
main:	[	281473125940960	]
sub new:	[	281473104359872	]
sub new:	[	281473079181760	]
sub new:	[	281473054003648	]
sub new:	[	281473095967168	]
sub new:	[	281473045610944	]
sub new:	[	281473112752576	]
sub new:	[	281473121145280	]
sub new:	[	281473062396352	]
sub new:	[	281473070789056	]
sub new:	[	281473087574464	]
5-->281473112752576
sub hand over:	[	281473112752576	]
8-->281473087574464
6-->281473062396352
5-->281473121145280
sub hand over:	[	281473121145280	]
4-->281473045610944
1-->281473079181760
1-->281473054003648
1-->281473095967168
...
0-->281473070789056
wake
sub hand over:	[	281473079181760	]
sub hand over:	[	281473104359872	]
sub hand over:	[	281473054003648	]
sub hand over:	[	281473062396352	]
sub hand over:	[	281473045610944	]
sub hand over:	[	281473087574464	]
sub hand over:	[	281473070789056	]
sub hand over:	[	281473095967168	]
```

<br/>

> 从打印的结果来看, 是多线程抢占printf的顺序, 不用管, 逻辑上是对的
>
> 结论: 可以发现每个信号只能唤醒1个线程, 其他的线程不会醒来. 所以使用全局变量没有问题. 利用这个特性可以做出条件变量的通知机制.
...
### sigwait屏蔽信号处理问题
- sigwait会屏蔽用户的信号处理, 考虑这种情况: 若有2个线程, 1个在sigwait某个信号; 另一个已经设置了相同信号的处理, 那么这种情况下, 当sigwait被该信号唤醒时, 另1个线程会不会执行信号处理呢? 书籍上所说的是不确定, 可能sigwait的线程被唤醒, 也可能将信号发送给其他线程
...
int main(int args, char** argv){
    ...
    //pthread_sigmask(SIG_BLOCK, &s, &os); // 不能在这里屏蔽, 因为后续创建线程时会继承当前线程屏蔽字, 导致后面的线程不响应信号
#if 1

	thread([]{
	    print_thread("sub new sigwait");
	    sigset_t s;
	    sigemptyset(&s);
        sigaddset(&s, SIGINT);
        int n;
...
> 上述程序运行后, 主线程屏蔽了SIGINT信号, 其他2个线程一个等待, 一个无限循环(<font color=green>可以信号中断处理</font>). 最后的结果是, 系统是唤醒了sigwait

```shell
./main
main:	[	281473055194848	]
sub new pause:	[	281473042006464	]               
sub new sigwait:	[	281473050399168	]
^Csigwait wake                                      # 发送信号, 直接唤醒了sigwait
^Csub hand over:	[	281473042006464	]           # 子线程虽然在循环中, 但还是被信号打断, 处理信号
^Csub hand over:	[	281473042006464	]
^Csub hand over:	[	281473042006464	]
```
...
### sigwait实现简单的mutex
- 在第10章节, 曾用`sigsuspend`实现了一个简单的mutex机制, 这里改用sigwait在2家平台上做一下测试
```cpp
#include<unistd.h>
#include<iostream>
#include<signal.h>
...
static constexpr auto NUM = 100ull;
static constexpr auto UINT= 10000ull;

static auto glb_count = 0ull;
...
static inline void print_thread(const char* info = "main"){
    fprintf(stderr, "%s\ttid[\t" F_TID "\t]\n", info, pthread_self());
}

static inline void sig_hand(int no){
    //fprintf(stderr, "sig_hand:%d\ttid[\t" F_TID "\t]\n", no, pthread_self());
}

static inline sigset_t sig_set(int no = SIGINT){
    sigset_t result;
    sigemptyset(&result);
    sigaddset(&result, no);
    return result;
...
int main(int arg, char** argv){
    ...
    for(auto i = -1; ++i < NUM;){
        thread([]{
            ...
            auto tmp_count = 0ull;
            auto sig_ignore_int = sig_set();
            int n = 0;
            auto loop = UINT;
	    int tmp = 0;
            for(auto i = -1; ++i < loop;){
                pthread_sigmask(SIG_BLOCK,&sig_ignore_int,nullptr);
		        sigwait(&sig_ignore_int, &tmp);
                //sigsuspend(&o_set);
                for(auto j = 0ull, n = UINT / loop;j < n;){
                    ++glb_count;
                    ++tmp_count;
                    ++j;
                }
                while(pthread_kill(mid, SIGUSR1)){
                    perror("pthred kill");
                    continue;
                }

                fprintf(stderr, "thread[\t" F_TID "\t]\t add:[\t" F_ULL "\t]\n", pthread_self(), tmp_count);
            }
        }).detach();
    }

    auto sig_ignore_usr = sig_set(SIGUSR1);
    auto sig_ignore_int = sig_set();
    auto sig_ignore_usr_int = sig_set();
    sigaddset(&sig_ignore_usr_int, SIGUSR1);
    pthread_sigmask(SIG_BLOCK, &sig_ignore_int, nullptr);

    sleep(5);

    auto kill_count = 0ull;
    int tmp = 0;
    while(glb_count < UINT * NUM){
        kill(pid, SIGINT);
        ++kill_count;
        pthread_sigmask(SIG_BLOCK, &sig_ignore_usr_int, nullptr);
	    sigwait(&sig_ignore_usr, &tmp);
        //sigsuspend(&sig_ignore_int);
    }

    cout << "kill_count:" << kill_count << endl;
    cout << "result:" << glb_count << endl;
...
> 上述程序是将第10章([#sigsuspend实现简单的mutex](./010_信号.md))sigsuspend实现的mutex改成sigwait, 当时在2平台上测试时, 只有Linux上表现出正确的行为, 在Mac上子线程可能没有被唤醒. 但这里改成sigwait后, 2家平台的行为是一样的. 同样的在程序中通过控制`loop 和 n`的比例, 来达到控制线程的吞吐量.


<br/>

### 线程发送信号
- 在前面的小节[线程与信号](#signal-thread)中提到过信号可能随机派到进程中的某个线程, 前面小节的多个案例也体现出了这一点. 但在2大平台上, 信号派发到主线程的频率要很高. POSIX声明了对线程信号的操作
...
> 同kill一样, 当信号为0时, 该函数表示检查指定的线程是否存在(<font color=green>kill则是检查对应的进程是否存在</font>). 如果指定的信号默认是终止进程, 则在没有指定信号处理时, 整个进程将被终止.
>
> PS: SIGALRM信号是进程资源, 所有的线程都共享它, 所以任何一个线程修改了时钟, 则所有的线程都共享.
>
> 可以将上述程序中的`kill`修改为`pthread_kill`, 但程序在逻辑上要改很多地方. 下面的案例:
...
atomic_flag      lock_tids{false};

static constexpr auto NUM = 20ull;
static constexpr auto UINT= 100000ull;
...
int main(int arg, char** argv){
    ...
    for(auto i = -1; ++i < NUM;){
        thread([]{
            print_thread("start");
            while(lock_tids.test_and_set(memory_order_acq_rel)){
                THREAD_YIELD();
                continue;
            }
            ...
            for(auto i = -1; ++i < loop;){
                pthread_sigmask(SIG_BLOCK,&sig_ignore_int,nullptr);
                while(sigwait(&sig_ignore_int, &tmp));                          // __code_add_self
                //sigsuspend(&o_set);
                for(auto j = 0ull, n = UINT / loop;j < n;){
                    ++glb_count;
                    ++tmp_count;
            ...
            erase(tids, pthread_self());                                        // __code_remove_self
            lock_tids.clear();
        }).detach();
    }
    ...
    auto sig_ignore_usr_int = sig_set();
    sigaddset(&sig_ignore_usr_int, SIGUSR1);
    pthread_sigmask(SIG_BLOCK, &sig_ignore_int, nullptr);                   // __code_0
    ...
    while(glb_count < UINT * NUM){
        ...
        if(tids.size()){
            default_random_engine e(random_device{}());
            uniform_int_distribution<int> r(0, static_cast<int>(tids.size())-1);
            auto rand_id = tids[r(e)];                                      // __coed_random_tid

            while(pthread_kill(rand_id, SIGINT)){
                perror("main kill");
                continue;
            }
            lock_tids.clear();
            ++kill_count;
            pthread_sigmask(SIG_BLOCK, &sig_ignore_usr_int, nullptr);
            while(sigwait(&sig_ignore_usr, &tmp));

            // 说明曾经有线程, 现在所有的线程都结束了, 直接结束
        }else if(tids.capacity()){
            lock_tids.clear();
            break;
        }else
            lock_tids.clear();
...
> 程序说明: 和上一小节程序不同的是: 此时的主线程不再等待所有的线程都创建完毕.  用CAS(`lock_tids`)来互斥访问数组(`tids`). 主线程从数组中随机出某个线程`__code_random_tid`,然后调用`pthread_kill`单独对它发送信号. 子线程和前面案例的区别主要是: 1. 线程启动后,直接将自己加入到数组中(`__code_add_self`); 2. 累加完成后, 将自己从数组中移除(`__code_remove_self`).  经过测试, 出现了这样一个问题: <font color=red>全局变量的累加是安全的. 但是最后主线不能顺序结束, 必须用`Ctrl + C`来唤醒子线程, 原因就是`pthread_kill`出现了没有唤醒线程的情况</font>. 
...
### 书籍上的案例
- `12-16`的程序是完善第10章(`10-23`)的通信机制. 这里简单通过截图的方式将展示出来, 如下:

---

![10-23](./assets/11_12_15.png)

---

![12-16](./assets/11_12_16.png)

---

<br/>

> 第10章的程序是测试sigsuspend被唤醒. 进程首先注册了`SIGINT`和`SIGQUIT`的信号处理. 接着屏蔽了2种信号. 然后通过判断全局的退出标识(`quitflag`)来决定要不要挂起(`sigsuspend`). 全局标识使用了`sig_atmoic_t`, 在信号处理中, 这种变量的修改是原子的, 保证即使是多线程同时进入了同一个信号处理函数也是修改互斥的. 当发生了`SIGQUIT`信号时, 在Linux中是先调用信号处理函数, 然后唤醒sigsuspend, 在信号处理中已经将退出标识置1, 所以进程会结束循环, 后面退出了.
>
> 而第12章这里的程序的逻辑也是一样的. 但全局的退出标识是用`mutex`来互斥访问的. 主线程会等子线程唤醒(`pthread_cond_signal`). 而子线程的任务就是处理信号, 它从主线程中被创建时, 继承主线程的信号屏蔽字的那一时刻, 屏蔽了`SIGINT`和`SIGQUIT`. 但子线程启动后,直接调用`sigwait`来等待这2信号, 当检测到`SIGQUIT`时, 通过互斥量来修改`quitflag`, 然后唤醒主线程, 主线程就退出了. 整个进程也就退出了
>
> PS: 第2种实现的好处就是: 用单独的线程来处理信号, 主线程不用关心自己被信号的干扰. 并且线程的调度效率要比从信号切换到进程高
...
### 线程和fork
- fork函数将当前进程下的所有内存复制一份到子进程中. 这里的内存包括: 1. 代码区(`共享`); 2. 堆栈; 3. 变量区(`全局,静态等待`); 4. 文件表项(`共享.回忆第3章系统IO`). ...; OS为了效率会作cpw(`copy on write`). 当子线程要修改某个共享内存的数据时, OS会单独copy出一份内存出来给子线程, 这样2者就不会有影响. 但这里容易忽略一个致命的问题: 锁!!!;
    - 首先是文件锁. 虽然现在还未系统的学习文件锁, 但应当明白, 文件锁肯定是通过`inode`实现的, 不同进程引用的同一物理文件, 它们的inode是文件系统中的同一个. 所以才能实现不同进程地同一文件的互斥读写
    - 互斥锁. 这里有个疑问: 子进程和父进程最终结果是数据是内存隔离的, 为什么还要考虑这种锁资源呢? 是因为如果子进程在被fork出来前, mutex可能已经上锁了, 则子进程启动后, 相应的自己的mutex也是上锁状态, 这样会出现当子进程利用这个锁去互斥访问数据时, 会死锁. 

> 只有当进程被fork出来后, 立即调用`exec`变为不同于父进程的其他程序, 才不用考虑这些锁资源的释放. 放在现在的多线程环境问题也是一样的. 需要说明一点: 在多线程环境下, 某个线程A调用fork时,产生的子进程的TCB是由创建它的那个线程A决定. 新创建的子进程中只有1个线程, 它并不包含原来父进程下所有线程. 下面先用代码演示出单进程下的互斥锁死锁问题
...
int main(int args, char** argv){
	...
	if(f == 0){
		cout << "thread:" << pthread_self() << endl << flush;
		g_lock.lock();
		cout << "working...\n";
		g_lock.unlock();
		cout << "over\n";
...
```shell
 ./main
thread:0x200884240          # 子进程创建
c:268435455                 # 主进程计算值后,打印
                            # 主进程pause, 挂起
```

<br/>

> 该程序没有实际效果, 只用来演示子进程被fork后, 死锁的问题. 发现子进程无法打印`woring`, 因为它从父进程fork时, 锁已经被持有了. 当然如果子进程不做其他事情, 直接exec成其他程序, 就不用关心这种问题. POSIX规定, 在fork后的子进程在exec之前 <font color=red>可以调用其他异步信号安全的函数</font>(`回想第10章, 这些函数一般都是系统调用`).
...
typdef void(* clear_call_back)(void);
int pthread_atfork(clear_call_back prepare, clear_call_back parent, clear_call_back child);

// 成功返回0. 其他返回错误编号
```

> 该函数是在调用fork前所注册的, 它告诉fork怎么处理锁. 在fork调用的过程中会回调所注册的函数
>
> `prepare`: fork内部,创建子进程前回调. 它一般和用来获取父进程中所定义的所有锁(`即持有锁`)
> 
> `parent`: fork内部,创建完子进程后, 返回到父进程前调用(<font color = green>注意它的上下文是父进程</font>). 它一般用于对prepare中的锁进行解锁
>
> `child`: fork内部,创建完子进程后, 返回到子进程前调用(<font color = green>注意它的上下文是子进程</font>). 它一般也用于对prepare中的锁进行解锁
> 
> 这里先测试出, 这些回调函数的调用时机.
...
static inline void print_thread(const char* info = "main"){
	fprintf(stdout,"%s thread:%p\n", info, pthread_self());
}


static inline void prepare(void){
    glb = 20;
	print_thread("prepare");
}
static inline void parent(void){
    glb = 30;
	print_thread("parent");
}
static inline void child(void){
    glb = 40;
	print_thread("child");
}
...
int main(int args, char** argv){
    ...
    pause();                    // MAC环境下, 程序运行后, 这里直接被唤醒, 目前未去探究原因
...
```shell
./main
main thread:0x20446a240
prepare thread:0x20446a240          # __print_sequence_0
parent thread:0x20446a240           # __print_parent
fork main
child thread:0x20446a240            # __print_child
sub thread:0x20446a240
```

> 经过测试发现`prepare`在fork时首先被调用. 至于parent和child谁先调用, 取决于调度器. 程序中子进程中线程id和父进程线程id一样, 这是因为fork后子进程id继承了父线程. 但他们的pid是不一样的, 这个不多说了. 
>
> PS: 虽然存在cow(`copy on write`), 但这3个回调不会因为对共享内存的修改, 出现回调函数被调用2次的情况. 如程序中的child, 虽然子进程已经被创建, 但在child函数中对全局变量修改时, OS复制glb的内存单独给子进程修改, 但并不会出现child函数再调用1次的情况. 
>
> 这3个函数的正确的用法并不局限于参数的赘述. 并不一定非要在`perpare`中加锁, 然后在`parent`和`child`中解锁. 要清楚的明白, 在要fork出子进程时, 有没有影响子进程的锁. 若是有的话, 应该在child函数中直接解锁而不用管此刻父进程的锁状态. 下面的案例是对前面死锁的解决方案. 其中prepare和parent直接忽略(`传递nullptr`). 因为在fork时, 清楚的知道`g_lock`已经上锁. 所以为了不影响子进程, 就直接注册子进程的清理, 这样返回到子进程时, 锁已经解开了. 同时因为在child中对`g_lock`作了修改, OS已经将这块内存单独复制给了子进程, 所以对父进程未有影响
...
### `pthread_atfork`的嵌套
-  像`atexit`函数一样, 也可以注册多个锁清理程序. 以书籍介绍来给出调用顺序:
    1. prepare是以入栈注册, 出栈的调用顺序来依次被执行. 即先注册后调用
    2. parent和child是以入队顺序调用. 即先注册先调用
...
template<int seq>
static void fork_cbk(void){
    // 这里 seq % 2 的目的是以分组形式展示
    // 数组下标 0, 2, 4表示的第1次注册
    //          1, 3, 5表示的第2次注册
    fprintf(stderr, "%s %d\tthread[%lu]\n", INFO[seq], seq % 2, pthread_self());
}

static void p(void(*call)(void)){}

#define CBK(N) fork_cbk<N>

int main(int, char**){
    pthread_atfork(nullptr, nullptr, nullptr);

    pthread_atfork(CBK(0), CBK(2), CBK(4));
    pthread_atfork(CBK(1), CBK(3), CBK(5));
...
```shell
./main
main[281472988516064]
prepare 1	thread[281472988516064]
prepare 0	thread[281472988516064]
parent 0	thread[281472988516064]
parent 1	thread[281472988516064]
main over
liubo-linux% child 0	thread[281472988516064]
child 1	thread[281472988516064]
sub over
```

<br/>

> 结论: `prepare 1`后注册的先打印, `parent 1`和`child 1`先注册的先打印. 
>
> PS: 书籍上所赘述的看起来很繁琐, 如图:
...
![pthread_atfork](./assets/11_12_17.png)

---

> 书籍上所要表述的意思是: `prepare`和`child parent`的调用顺序可以保证正确的加解锁顺序, 这样不会造成死锁.
...
```txt
    ...
    注册的顺序是
        pthread_atfork(prepare_0, parent_0, child_0)
        pthread_atfork(prepare_1, parent_1, child_1)

    根据 pthread_atfork在嵌套时的调用顺序:
        prepare_1 --> b.lock()
        prepare_0 --> a.lock()

        parent_0  --> a.unlock()
        parent_1  --> b.unlock()            // 父进程中正确解锁
        
        child_0   --> a.unlock()
        child_1   --> b.unlock()            // 子进程中也正确解锁
```

> 书籍上对`pthread_atfork`的解释都是基于: 在`prepare`中加锁, 在`parent`和`child`中解锁. 但要明白的是, 用户调用fork的时机并不确定. 很多情况是在调用fork前就已经对互斥量加过锁了, 在调用fork后, 要保证子进程不死锁, 则应该注册`child`函数, 然后根据程序当前的加锁逻辑来解锁. 
>
> `pthread_atfork`对于条件变量无法做清理, 因为没有清理条件变量的接口. 
...
![pthread_atfork](./assets/11_12_18.png)
...
[^ann-tcb-0]: `thread control block`线程控制块, 用来描述线程状态的数据结构. 对应的也有进程控制块(`pcb process control block`), 按正常逻辑去实现的话, 每个进程应会有`tcb-table`的逻辑表, 用来记录当前进程中所有线程. tcb可由内核维护, 也可由用户所编写的线程库函数维护. 如今pthread的在Unix环境中基本是封装的内核线程接口. 当调用`pthread_detach`后, 线程会分离不阻塞, 分离线程结束后, 自动被回收(`tcb被销毁`). 若对一个线程不做`pthread_join或pthread_detach`, 则线程的tcb不会被回收. Linux的环境下线程和进程其实没有本质的区分, 他们的结构是一样的, 这个在本章的学习过程会逐渐说明


[^ann-thread-stack]: 线程调用栈, 由内核指定. 多线程时, 每条线程的栈是独立的, 但它们的堆是共享的(`堆属于进程,所以线程可以共享`). 常常说针对线程所说的堆栈, 严格来说只是表示线程的函数调用栈, 后面会有线程堆的说明.

[^ann-abi-0]: Application Binary Interface, 编译器对接的系统调用接口(`二进制`), 即每种操作系统的ABI是不一样的. 有的编译器可能还会直接对接到底层的库接口, 如gcc可能调用glibc的接口

[^ann-errno-1]: 它能作为全局变量而不考虑并发的原因是, 当时已经有了进程模型, 线程未提出. 并发的机制也是内存隔离的进程, 所以没有问题. 后来出现线程模型后, 标准C要求errno在线程间操作互不影响, 所以相应的也就成了线程变量

[^ann-async]: 这里所说的异步不同于并发. 以信号来说, 当产生硬件中断时, 当前CPU收到中断请求, 挂起当前的任务(`当前线程`) 去查找中断向量表, 然后处理, 信号就是在内核处理程序中生成的信号序列, 然后转发到当前线程注册的信号处理函数. 也就是说当信号发生时, 当前线程一定处于挂起状态, 等到被调度时, 会进入到信号处理阶段. 并发是所有的线程都在工作. 注意一点, 多核下, 信号发生时, 只有当前cpu所在的线程才会挂起, 其他线程并不会挂起

[^ann-main-thread]: 所谓的主线程由笔者自己定义, 即main所在的线程

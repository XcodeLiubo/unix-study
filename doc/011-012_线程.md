...
### 并没有真正意义上的线程变量
- 现在来思考这样一个问题: 定义在函数体外的线程变量的地址? 调用`pthread_setpecific`存储的malloc开辟的地址?
- 对于`_Thread_local`变量, 

<br/>

> 注意在Mac上实现的机制是直接调用`pthread_setspecific`而没有所谓的(`TLS thread local store`), 猜想大致过程是当编译器发现用户在某个线程中访问`_Thread_local`声明的变量时, 调用相关的ABI接口为该变量创建内存,这个过程只会有一次, 然后再通过`pthread_setspecific`关联该内存到TCB中

...
```
> 上面这个程序在信号被注册前(`__code_signal`)就向内核告知 <font color=red>阻塞SIGINT</font>(`__code_proc_mask`). 所以最后的结果是 <font color=red>程序在运行的过程在, 在终端连续摁Ctrl+C是没有反应的</font>. 
...
> 
> 这个问题的猜想会继续下去, 在第`11_12`章的线程与信号的章节, 做了更合理的猜测


<br/>

### sigprocmask中set为空时
- 当要解除阻塞并且指定<font color = deeppink>第2参数set为NULL时, 该函数并不做任何处理</font>
```cpp
#include<unistd.h>
#include<signal.h>
#include<iostream>
...
int main(int args, char** argv){
    ...
    cout << bitset<32>(*(uint32_t*)&oset) << endl;  // __code_print_block_before
   
    sigemptyset(&set);
    sigaddset(&set, SIGINT);


    sigprocmask(SIG_BLOCK, &set, nullptr);          // __code_block

    sigprocmask(SIG_SETMASK, nullptr, nullptr);     // __code_expect_unblock

    sigprocmask(0, 0, &oset);
    cout << bitset<32>(*(uint32_t*)&oset) << endl;  // __code_print_unblock_after
...
> 上述程序在`__code_expect_unblock`传递的 <font color=red>set是NULL</font>, 则函数并不会解除阻塞. 所以`__code_print_block_before`和`__code_print_unblock_after`打印的是不一样的
...
```shell
./main
00000000000000000000000000000000            # __code_print_block_before
00000000000000000000000000000010            # __code_print_unblock_after 并没有解除阻塞
^C^C^C^C^C^C^C^C^C^\[1]    33968 quit       ./main # 摁下 Ctrl+\
```
...
<img src="assets/10_24.gif"/>

<br/>

> 若将前一小节的中的`__code_reset_original`中改为`sigprocmask(SIG_SETMASK, nullptr, nullptr)`, 则结果是不会响应未决信号
...
|`SA_ONSTACK`|0x0001|将信号发送到指定栈上的进程(`在下章的线程中会在Linux环境下讨论这个`)|
...
int main(int args, char** argv){
    ...
    signal(SIGUSR1, SIG_IGN); // 这一步并不会修改mask的屏蔽位, 所以还是原来的
...
> 在Mac和Linux上分别测试上述修改的程序(<font color=red>需要说明的是在Linux上SIGUSR1的编号是10, `sigset_t`的大小是128 * 8大小, 这里为了方便, 直接取的32位, 包含了标准的32个信号</font>), 测试结果:
...
- 现在来思考一个问题: sigsuspend内部实现中会不会处理未决信号?
...
static inline void print_mask(const char* info = ""){
	union{
		sigset_t s;
		uint32_t val;
	}tmp;
	sigemptyset(&tmp.s);
	pthread_sigmask(0, 0, &tmp.s);
	ostringstream str;
	str << info;
	str  << bitset<32>(*&tmp.val) << endl;
	cout << str.str().c_str();
}
...
> 上述程序通过2种方式测试: 
>   1. 程序运行后等待2秒以上
>   2. 程序运行后在2秒内产生SIGINT, 再等待到2秒的alrm信号
>
> 第1种测试结果: 2秒后, 产生alrm信号, 打断了sigsuspend, 但通过打印结果, 先调用了信号处理函数, 再从sigsuspend返回
>
> 第2种测试结果: 2秒内键入`Ctrl + C`并不会结束程序(此刻SIGINT成了未决信号), 因为sigsuspend指定了不能被SIGINT唤醒. 所以程序2秒后, sigsuspend被alrm唤醒, 程序先调用了alrm的信号处理, 然后直接结束了, 连`wake`都没有打印. 因为alrm信号处理完毕后, 进程再次回到内核, 然后等待调度, 再被调度时, SIGINT有未决信号, 但发现没有注册SIGINT所以直接结束了
>

```shell
./main
00000000000000000000000000000000                    # 程序运行先输出初始化状态下的mask
sig alarm:	00000000000000000010000000000010        # 2秒后产生的alrm信号, 进入信号处理. 同之前分析, Linux先调用信号处理函数, 再还原信号. 所以 第1位(SIGINT为1), 这个1是sigsuspend调用时设置的SIGINT为屏蔽 
wake                                                # sigsuspend返回程序结束

./main
00000000000000000000000000000000
^C^Csig alarm:	00000000000000000010000000000010    # 2秒内多次产生`SIGINT`信号, 但因为sigsuspend指定不被它唤醒, 所以没效果
                                                    # 等待时间到了2秒, 产生alrm信号, 进入信号处理, 然后回到内核, 下一次调度
                                                    # 因为有SIGINT未决,但没有信号处理程序, 所以直接结束进程

```

<br/>

> 同样的在Mac上测试结果是不一样的, 第2种测试模式中Mac被alrm唤醒后, 直接结束进程, 连`alrm`信号都未响应. 原因就是2家实现不同了. alrm唤醒sigsuspend, 先还原SIGINT(`前面小节所学`), 还原信号的操作可能导致直接进入内核, 然后下一次调度时发现有2个未决信号(`SIGINT, SIGALRM`). 虽然标准对于信号的调用顺序没有要求, 但OS一般会先处理异常信号, 所以这里因为SIGINT没有处理函数, 所以异常的优先级高, 直接先调用它结束了. 下面是测试结果
...
./main
00000000000000000000000000000000
sig alarm:	00000000000000000010000000000000            # 2秒后alarm信号, 先还原了SIGNT(第2个bit位为0), 再调的信号处理
wake


 ➜ lb ./main
00000000000000000000000000000000                                
^C^C^C^C^C^C^C                                          # 2秒内, 产生多闪SIGINT, 但没有反应, 2秒到了后, 产生alrm, 唤醒
                                                        # sigsuspend, 先还原信号,进入内核, 下一次调度2个未决信号
                                                        # 先处理了SIGINT直接结束了
```

<br/>

### sigsuspend实现简单的mutex
- 因为sigsuspend原子的设置线程的mask, 并每次只能唤醒一个线程, 所以理论上可以实现锁机制, 但实际上在MAC上不可靠(<font color = green>并不是不安全</font>)
...
int main(int arg, char** argv){
    ...
    for(auto i = -1; ++i < NUM;){
        thread([]{
            ...
            int n = 0;
            auto loop = UINT / 10;              // __code_loop
            for(auto i = -1; ++i < loop;){
                pthread_sigmask(SIG_BLOCK,&sig_ignore_int,nullptr);
                sigsuspend(&o_set);             
                for(auto j = 0ull, n = UINT / loop;j < n;){     // __code_n
                    ++glb_count;
                    ++tmp_count;
                    ++j;
                }
...
> 上述程序中, 主线程屏蔽自己的`SIGINT`, 因为主线程要自己发送`SIGINT`然后由唤醒某个子线程(<font color = green>若主线程不屏蔽该信号, 则内核基本会将该信号派发到主线程来处理, 所以这样能保证这种信号发送到子线程</font>). 它自己接收`SIGUSR1`, 目的是子线程在一轮叠加结束后发送`SIGUSR1`唤醒主线程, 然后主线程再发送`SIGINT`, 进入2线程这种交互的循环操作. 但实际测试过程中发现子线程都存在不被唤醒的情况(`MAC`), 这样就需要手动发送`SIGINT`让其中一个线程起来工作一下(`手动时不要连续发, 线程就不安全了`), 进入累加流程. 但最后测试的结果是没有问题的.
>
> `__code_sleep`的目的是等待所有的线程创建完毕, 这样的目的只是为了测试, 因为没有去处理kill发送后, 还没有线程的情况.
>
> `__code_loop` 和 `__code_n`控制吞吐量, 若`__code_loop`接近`UINT`, 则每条线程叠加的次数相对较少, 调用sigsuspend的频率就大, 则OS调度唤醒线程的开销会很大. 所以让一条线程每一轮叠加的次数相对多, 则效率就高一点
>
> PS: 上述程序中使用了`pthread_sigmask`, 这个是POSIX中对线程屏蔽字操作的接口,这一点将在第`10_11`的线程章节做学习. 对应的也有`sigwait`的比对. 需要再说明一点, 这里只是测试出sigsuspend的确只能唤醒某个线程, 但上述程序中某个子线程被主线程发送的SIGINT唤醒后, 在执行任务的过程中, 若同时由别的用户在交互中发送SIGINT(`键入Ctrl + c`), 则必定会唤醒其他的线程, 造成多线程对全局变量(`glb_count`)同时操作, 就不是线程安全的了.  作为上层的用户可以通过sigsuspend的机制来实现mutex锁机制. 后面线程的章节中glibc的mutex机制虽然不是借助sigsuspend实现的, 但原理是一样的, 它内部调用的是`futex`, futex也保证只唤醒1条线程, 所以从原理上来看, 可能通过sigsuspend来封装成mutex

<br/>

<img src="assets/10_34.gif"/>

<br/>

### sigsuspend实现简单的进程间通讯
- 前面实现了信号驱动案例, 同样的可以将这种模型扩展到进程间通讯

```txt
场景:
    
    一个简单的 C/S 模型
    client客户端向 server 发出各种请求, 增删改查文件
    server端管理文件, 给client端的请求提供数据操作
    

抽象:
    为了演示方便, 直接在同一个函数中使用fork
    父进程管理数据库文件
    子进程通过标准输入根据用户动作向父进程请求操作
...
#define CAT_ID(_id) _id, sizeof(_id)



// 通讯的文件
#define SOCK_FILE "./sock.io"
#define DATABASE_LOGIN "./login.table"
// 数据库
#define DATABASE_FILE "user.table"

// 父子进程中接收登录和退出的信号
#define SOCK_SIG  SIGUSR1

// 父子进程中接收 增删改查信号
#define SOCK_CRUD SIGUSR2
...
enum ResOper{
    RES_LOGIN   = REQ_LOGIN,
    RES_SELECT  = REQ_SELECT,
    RES_INSERT  = REQ_INSERT,
    RES_DELETE  = REQ_DELETE,
    RES_UPDATE  = REQ_UPDATE,
    RES_LOGOUT  = REQ_LOGOUT,
};
...
#pragma mark - 全局变量
static pid_t ref_pid;           // 子进程是0, 父进程则是子进程的id
static FILE* sock_file;
// block set and original set
static sigset_t bset, oset;


#pragma mark - 函数
static void sock(void*, size_t, auto (*)(const void*, size_t, size_t, FILE*)->size_t, FILE* = nullptr);
static void sock(void*, size_t, auto (*)(void*, size_t, size_t, FILE*)->size_t, FILE* = nullptr);

static FILE* db_file(void);

static void do_main(void);
static void receive_login(const Req&);
static void crud(void);

static void do_child();
static void login(Person& user);
static void wait_user_req(void);

static inline void copy_id(char r[], size_t r_len, string id){
    memset(r, 0, r_len);
    auto id_c = id.c_str();
    auto i_len = strlen(id_c);
    // 因为不填充 '\0', 所以这里自己填充
    strncpy(r, id_c, min(r_len, i_len));
}
...
int main(int args, char** argv){
    ...
    sigprocmask(0, nullptr, &oset);
    sigemptyset(&bset);
    sigaddset(&bset, SOCK_SIG);
    sigaddset(&bset, SOCK_CRUD);
...
static void do_main(void){
    ...
    // 登录和退出使用的信号
    signal(SOCK_SIG, [](int no){
        Req req;
        sock(&req, sizeof(Req), fread);
        if(req.op == REQ_LOGIN){
    ...
    st.sa_sigaction = [](int signo, siginfo_t* info, void* ctx){
        if(info->si_pid != ref_pid){
            Res r;
            r.is_suc = info->si_code == CLD_EXITED;
            sock(&r, sizeof(Res), fwrite);
            kill(ref_pid, SOCK_CRUD);
            return ;
        }
        if(signo == SIGCHLD){
...
#pragma mark - 接收到登录
static void receive_login(const Req& req){
    Res r;
    r.op = (ResOper)req.op;

    auto tmp_login_file = fopen(DATABASE_LOGIN, "r");

    string cat(req.id);
    cat = cat + " " + req.user.pwd + "\n";

    while(1){
        char line_str[0x30] = {0};
        flock(fileno(tmp_login_file), LOCK_EX);         // error, 只读不行
        fgets(line_str, 0x30, tmp_login_file);
        flock(fileno(tmp_login_file), LOCK_UN);

        if(feof(tmp_login_file))
            break;
        if(ferror(tmp_login_file))
            break;
        ...
        if(line == cat){
            r.is_suc = true;
            copy_id(CAT_ID(r.result.name), req.id);
            break;
        }
    }

    sock(&r, sizeof(Res), fwrite);
    kill(ref_pid, SOCK_SIG);
    fclose(tmp_login_file);
}
...
static void crud(void){
    ...
    switch (req.op) {
        ...
        case REQ_DELETE:
        case REQ_SELECT:{
            flock(fileno(db_file()), LOCK_EX);
            fseeko(db_file(), 0, SEEK_SET);
            while(1){
                fread(&r.result, sizeof(r.result), 1, db_file());

                if(feof(db_file()) || ferror(db_file()))
                    break;

                if(string(r.result.name) == string(req.id)){
                    r.is_suc = true;
                    break;
                }
            }
            ...
            // 若当前是查询, 直接退出循环, 去发信号
            if(req.op == REQ_SELECT)break;


            // 操作不存在的用户(删除和更新), 直接发信号
            if(r.is_suc == 0) break;



#if 0       // Mac上的sed 更改一整行, 命令中必须换行, 这里拼接好字符串后, sed不能识别, 放弃
            ...
            // Mac上删除匹配行
            sed -i '' '/tierry/d' user.table
#endif

            // 删除借用 sed命令
            if(req.op == REQ_DELETE){
                string cmd("sed -i \"\"");      // sed -i ""
                cmd += " ";
                cmd += "\"/";                   // sed -i "" "/
                cmd += req.id;                  // sed -i "" "/tierry
                cmd += "/d\"";                  // sed -i "" "/tierry/d"
                cmd += " ";
                cmd += DATABASE_FILE;
            ...
            // 更新
            flock(fileno(db_file()), LOCK_EX);
            fseeko(db_file(), -(ssize_t)sizeof(req.user), SEEK_CUR);
            fwrite(&req.user, sizeof(req.user), 1, db_file());
            fflush(db_file());
            flock(fileno(db_file()), LOCK_UN);
            r.is_suc = !ferror(db_file());
...
static void do_child(){
    ...
    signal(SOCK_SIG, [](int no){
        Res r;
        sock(&r, sizeof(Res), fread);
        if(r.is_suc){
            copy_id(CAT_ID(user.name), r.result.name);
    ...
    signal(SOCK_CRUD, [](int no){
        ...
        if(r.is_suc){
            switch (r.op) {
                case RES_SELECT:
                    cout << "搜索结果:\t" << r.result.name << "\t" << r.result.age << "\t" << r.result.addr << endl;
                    break;
    ...
    while(1){
        ...
        cout << "***********************************************************\n"
        <<      "                person manager                             \n"
        <<      "                "<< REQ_SELECT<<": select(name)            \n"
        <<      "                "<< REQ_INSERT<<": insert(name age addr)   \n"
        <<      "                "<< REQ_DELETE<<": delete(name)            \n"
        <<      "                "<< REQ_UPDATE<<": update(name age addr)   \n"
        <<      "***********************************************************\n";
...
static void login(Person& user){
    ...
    copy_id(CAT_ID(req.id), name);
    copy_id(CAT_ID(req.user.pwd), pwd);

    sock(&req, sizeof(Req), fwrite);

    kill(getppid(), SOCK_SIG);
}
...
static void wait_user_req(void){
    ...
    vector<string> vecs;
    string tmp;
    auto buf = stringstream(input);
    while(getline(buf, tmp, ' ')){
        vecs.push_back(tmp);
    }

    Req r;
    r.op = (ReqOper)atoi(vecs.begin()->c_str());
    copy_id(CAT_ID(r.id), vecs.at(1));

    switch (r.op) {
        case REQ_UPDATE:
        case REQ_INSERT:
            r.user.age = atoi(vecs.at(2).c_str());
            copy_id(CAT_ID(r.user.name), vecs.at(1));
            copy_id(CAT_ID(r.user.addr), vecs.at(3));
            break;
    ...
    sock(&r, sizeof(Req), fwrite);
    kill(getppid(), SOCK_CRUD);
}
...
static void sock(void* data, size_t len, size_t (* fun)(const void*, size_t, size_t, FILE*), FILE* stream){
    stream = stream?stream:sock_file;
    flock(fileno(stream), LOCK_EX);
    fseeko(stream, 0, SEEK_SET);
    fun(data, len, 1, stream);
    fflush(stream);
    flock(fileno(stream), LOCK_UN);
}

static void sock(void* data, size_t len, size_t (* fun)(void*, size_t, size_t, FILE*), FILE* stream){
    sock(data, len, (size_t(*)(const void*, size_t, size_t, FILE*))fun, stream);
}

static FILE* db_file(void){
    static atomic_flag once = {0};
    static FILE* db_f;
    while(atomic_flag_test(&once) == true){
        continue;
    }

    if(db_f == nullptr){
        db_f = fopen(DATABASE_FILE, "r+");
        fseeko(db_f, 0, SEEK_SET);
    }

    atomic_flag_clear_explicit(&once, memory_order_release);
    return db_f;
}
...
void init(){
    ...
    vec.push_back({20, "tierry", "猪窝"});
    vec.push_back({25, "liubo", "狗窝"});
    vec.push_back({30, "jerry", "鼠窝"});

    for_each(vec.begin(), vec.end(), [](Person& item){
        clearerr(db_file());
        fwrite(&item, sizeof(Person), 1, db_file());
        fflush(db_file());
        if(ferror(db_file())){
            perror("fwrite");
        }
...
程序会有2个进程, 但各司其职
    ...
    父进程:
        监听子进程发送的信号并处理
            SIGUSR1: 主要用于 父子进程登录和退出. 在本事例中并未实现退出
            SIGUSR2: 主要用于 父子进程间数据 增删改查
            父子进程通过 sock.io文件来传递登录的消息, 每次进程在访问该文件时, 都
            会加锁访问, 同里user.table也是一样的作用
            
            因为在同一台机器上, 直接使用的结构体来传递信息. 需要说明的是
            利用结构体传递信息时, 实际是上将结构体在内存的数据直接复制到磁盘上
            所以, 若结构体中是一个 int类型, 则在磁盘上查看时解析出给用户的格式
            是不对的. 但再从磁盘读出来到结构体时, 实际上是没有问题的
            
            程序中定义的 Person, 是自己计算的对齐, 如最后的字段eof
            默认情况下是4对齐. eof是为了方便, 之所以定义成int类型, 是为了在 声明Person
            变量时, 里面所有的数据是初始化的. 否则若eof是char, 则因为对齐的
            问题, 在栈上初始化一个Person变量时, 有部分的数据是未初始化的
            导致查看文件内容时, 会被混淆, 但实际上是没有问题的

            
            程序在做删除功能时, 直接使用的系统的环境变量 sed 命令, 所以
            监听了SIGCHLD信号, 在对比不是子进程发出时, 若进程是正常结束
            则统一认定为操作是成功的, 但其实是不严谨的
```
...
### SIGABRT与abort库函数
- `SIGABRT`和其他大多数信号一样终止进程. 单独使用时, 可以对其进行阻塞. 值得提出的是当它由`abort`函数触发时, 会直接忽略设置的阻塞, 从信号处理返回后, 直接结束进程
...
int main(int args, char** argv){
    ...
    cout << "blocked SIGABORT\n";

    do{
        cout << "test kill[k] raise[r] abort[a] quit[q]:";
        cin >> op;

        if(op  == "k"){
            sigset_t tmp_set;
            auto tmp = sigpending(&tmp_set);                    
            cout << bitset<32>(*(uint32_t*)&tmp_set) << endl;   // __code_pending_before
            kill(getpid(), SIGABRT);                            // __code_kill
            tmp = sigpending(&tmp_set);
            cout << bitset<32>(*(uint32_t*)&tmp_set) << endl;   // __code_pending_after
        }else if(op == "r"){
            raise(SIGABRT);                                     // __code_raise
        }else if(op == "a"){
            abort();                                            // __code_abort
        }else if(op == "q"){
            break;
        }else{
            cerr << "not found command\n";
        }
    }while(1);
...
> 先说结论 <font color=red>只有`abort()`能导致信号被响应</font>. 原因猜想一下: abort内部一定解除了`SIGABRT`的阻塞. 根据猜想的原因, 这样制定测试流程:

```txt
测试1
    进程运行后, 等待用户输入, 此时先不测试 abort的情况
        1. 输入 k, 则会调用kill
        2. 再输入r,则会调用raise
        3. 输入q, 直接退出
    ...
    猜测测试1结果:
        前2次输入后, 控制台并未打印出 信号处理函数的输出
        当输入q时, 结束循环. 此时解除阻塞
        但前2次的输入导致 SIGABRT的pending位为1, 所以会
        在`__code_unblock`时响应一次未决信号
```
...
./main
blocked SIGABORT
test kill[k] raise[r] abort[a] quit[q]:k
00000000000000000000000000000000                // __code_pending_before
00000000000000000000000000100000                // __code_pending_after 未决信号
test kill[k] raise[r] abort[a] quit[q]:q        // 等待输入, 并输入q, 导致循环结束, 响应一次未决信号
abort                                           // 未决信号响应
over                                            // 进程结束前最后的打印
```
...
测试2
    ...
    猜测测试结果:
        输入a后, 程序直接响应未决信号后, 不会打印 __code_over, 直接结束
```

<br/>

> 经过测试: 程序打印了2次`信号处理`, 推测原因是 <font color=red>abort内部解除阻塞时会先响应1次未决信号, 然后它内部主动调用一次信号处理</font>, 先看一下测试结果

```cpp
./main
blocked SIGABORT
test kill[k] raise[r] abort[a] quit[q]:k                // 输入k
00000000000000000000000000000000                        // __code_pending_before
00000000000000000000000000100000                        // __code_pending_after, 此时有未决信号, 并且SIGABRT还处理阻塞状态
test kill[k] raise[r] abort[a] quit[q]:a                // 输入a
abort                                                   // 第1次信号处理, 推测应该是abort内部先解除阻塞, 响应1次未决信号
abort                                                   // 第2次信号处理, 推测是aboart内部又自己调用了信号处理函数
[1]    34255 abort      ./main                          // 最后未打印 __code_over, 进程直接结束
```

<br/>

> 上述测试出现了2次`信号处理`, 作者猜测的顺序是:
>   1. 响应未决信号
>   2. 返回到abort后, 内部直接调用信号处理程序
> 至于第2点是因为abort函数并不返回到用户, 而是直接结束程序, 所以它内部会调用到`_exit`的系统调用. 下面用lldb调试
...
### 在 _exit的系统函数打个断点
(lldb) b _exit
Breakpoint 1: where = libsystem_kernel.dylib`__exit, address = 0x00007ff8003be2b0

### 在 sigprocmask打断点
(lldb) b sigprocmask
Breakpoint 2: where = libsystem_kernel.dylib`sigprocmask, address = 0x00007ff8003bb03c


### 运行
(lldb) r
Process 37243 launched: '/tmp/lb/main' (x86_64)


### 第1次捕捉到断点,  在main.cpp的 17行(调用了sigprocmask)
Process 37243 stopped
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
    frame #0: 0x00007ff80cf7b03c libsystem_kernel.dylib`sigprocmask
libsystem_kernel.dylib`sigprocmask:
->  0x7ff80cf7b03c <+0>:  movl   $0x2000030, %eax          ; imm = 0x2000030
    0x7ff80cf7b041 <+5>:  movq   %rcx, %r10
    0x7ff80cf7b044 <+8>:  syscall
    0x7ff80cf7b046 <+10>: jae    0x7ff80cf7b050            ; <+20>
Target 0: (main) stopped.

### 查看栈桢, 确实是由main中第17行手动调用
(lldb) bt
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
  * frame #0: 0x00007ff80cf7b03c libsystem_kernel.dylib`sigprocmask
    frame #1: 0x0000000100003f44 main`main(args=1, argv=0x000000030410afa8) at main.cpp:17:5    
    frame #2: 0x000000020001241f dyld`start + 1903


## 继续
(lldb) c
Process 37243 resuming
...
### 查看栈桢, 发现是由 abort调用, 说明abort内部先调用sigporcmask解除阻塞, 这一步一定会引起未决信号被调用
### ps: 在运行到断点前, main.cpp中的第20行产生了未决信号
(lldb) bt
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 2.1
  * frame #0: 0x00007ff80cf7b03c libsystem_kernel.dylib`sigprocmask
    frame #1: 0x00007ff80ceddb26 libsystem_c.dylib`abort + 92
    frame #2: 0x0000000100003f53 main`main(args=1, argv=0x000000030410afa8) at main.cpp:23:5
    frame #3: 0x000000020001241f dyld`start + 1903
...
### 第3次捕捉到断点, 但这1次是由 SIGABRT信号引起
Process 37243 stopped
* thread #1, queue = 'com.apple.main-thread', stop reason = signal SIGABRT
    frame #0: 0x00007ff80cf7e25e libsystem_kernel.dylib`__pthread_sigmask + 10
libsystem_kernel.dylib`:
->  0x7ff80cf7e25e <+10>: jae    0x7ff80cf7e268            ; <+20>
    0x7ff80cf7e260 <+12>: movq   %rax, %rdi
    0x7ff80cf7e263 <+15>: jmp    0x7ff80cf78cdb            ; cerror_nocancel
    0x7ff80cf7e268 <+20>: retq
Target 0: (main) stopped.

### 查看这1次的栈桢, 发现由 abort内部直接调用了信号处理
(lldb) bt
* thread #1, queue = 'com.apple.main-thread', stop reason = signal SIGABRT
  * frame #0: 0x00007ff80cf7e25e libsystem_kernel.dylib`__pthread_sigmask + 10
    frame #1: 0x00007ff80cfb67ab libsystem_pthread.dylib`pthread_sigmask + 9
    frame #2: 0x00007ff80ceddb3a libsystem_c.dylib`abort + 112
    frame #3: 0x0000000100003f53 main`main(args=1, argv=0x000000030410afa8) at main.cpp:23:5
    frame #4: 0x000000020001241f dyld`start + 1903
...
### 结论:
#### abort内部会调用sigprocmask解除阻塞, 然后直接调用信号处理程序
#### 疑惑的是 _exit的断点没有捕捉到
```
> 本次测试最终`abort`并未调用到`_exit`, 也没有出现`pthread_kill`相关的函数. 这个问题先放这里(<font color=green>以后回来看</font>). 


### abort刷新流?
- abort函数规定了程序进入退出流程, `ISO C`对于这种情况的退出并未做详细指定. 也就是说当调用abort时, 若此时还有未刷新的流, 则流可能刷新也可能不刷新.
> 对于abort调用POSIX也做了规定: <font color=red>若调用abort终止进程, 则所有打开标准IO流效果应与终止前对每个流调用fclose相同</font>. 说白了就是应当刷新流(<font color=green>但对于c++的文件流来说有一点问题, 这个在下节讨论一下</font>)
...
int main(int args, char** argv){
    ...
    auto write_file = fopen("a.txt", "w+");
    if(nullptr == write_file){
        perror("open");
        exit(-1);
    }

    auto tmp_file = tmpfile();
    if(tmp_file == nullptr){
        perror("tmpfile");

        // ignore write_file close
        exit(-1);
    }

    signal(SIGABRT, [](int no){ cout << "sig abort\n"; });      // __code_sigabort

    signal(SIGINT, [](int no){abort();});                       // __code_sigint

    char name_buf[0x100] = {0};

    ostringstream tmp_info;                                     // __code_tmp_info

    auto fd = fileno(tmp_file);
    fcntl(fd,F_GETPATH, name_buf);
    tmp_info << "临时文件路径:\n\t"<< name_buf << "\n访问模式:\n\t";



    auto ffl = fcntl(fd,F_GETFL);

    // 这里假设只有3种访问模式(其余的2种为了方便不考虑)
    // O_RDONLY, O_WRONLY, O_RDONLY
    bool canwrite = (ffl & O_ACCMODE) != O_RDONLY;
    bool canread = (ffl & O_ACCMODE) != O_WRONLY;
    ...
    if(canwrite){
        tmp_info << "will write to file:\n\t\"hello world\"\n";
        fputs("hello world", tmp_file);
        fflush(tmp_file);
        rewind(tmp_file);
    }
    if(canwrite && canread){
        tmp_info << "read from file:\n\t";
        fgets(name_buf, 0x100, tmp_file);
        tmp_info << name_buf;
    }

    cout << tmp_info.str() << endl;


    fputs("hello world", write_file);       // __code_puts_tofile
...
> 上述程序测试了:
>   1. abort被调用后, 会刷新标准IO流
>   2. 临时文件相关的操作
> 测试步骤:
>   1. 运行程序
>   2. 因为在编写代码时, 回顾了一下临时文件相关的, 所以在运行过程中会输出临时文件相关的信息. 现在先不考虑这些输出
>   3. 当向文件中写入内容后`__code_puts_tofile`,程序会阻塞住`__code_pause`
>   4. 此时向进程发送`SIGINT`(<font color=red>摁下Ctrl+C</font>)
>   5. 收到`SIGINT`后, 进程进入到`__code_sigint`, 并且立即调用`abort`
>   6. 第5步调用`abort`后, 进程先响应信号`__code_sigabort`, 然后结束
> 最后发现a.txt中被写入了内容, 程序中并未在`第3步后`刷新流, 也未在`第6步`的信号中刷新流, 但最后内容还是写入了文件, 所以abort函数内部在unix系统中会主动刷新流
> 该程序中有一部分是临时文件相关的打印, 一方面是回顾临时文件的接口, 另一方面也是将书籍中提到的拿出来作一下测试. Unix中的`tmpfile`并不是POSIX标准, 而是`ISO C`, 它内部的逻辑是创建一个唯一的文件, 然后立即`unlink`, 这样会导致用户从磁盘查找文件时, 文件是不存在的, 因为unlink后, 它就不存在文件系统中. 但当前进程正在引用它, 所以可以通过进程对它进行读写. 代码中先后测试了临时文件被打开的权限, 在合适的权限下时, 才会写入或读取
> 最后在程序中`fcntl`传入了一个`F_GETPATH`的选项, 这是MacOS自己扩展的, 并不是标准, 在Linux下没有

<br/>

--- 

<img src="assets/10_25.gif"/>

<br/>


### C++中的文件流和abort
- 上述程序若改写为C++的文件流, 则就有问题, 先看代码
...
int main(int args, char** argv){
    ...
    ofstream write_file("./a.txt", ios_base::trunc | ios_base::out);
    ...
    signal(SIGABRT, [](int no){ cout << "sig abort\n"; });
    signal(SIGINT, [](int no){abort();});

    write_file.write(argv[1], strlen(argv[1]));     // __code_write
...
<img src="assets/10_26.gif"/>

<br/>

> c++标准库过于复杂, 这里直接用lldb在`x86_64`下调试, 直接断点在`fwrite`. 经过测试可以发现: <font color=red>fwrite从`__code_write`后只被调用了一次, 并且传递的参数是`("h", 1, 1, file)`, 所以最后abort结束后, 文件的内容只有`h`这1个字符</font>
> 一般来说, abort由用户自己调用, 所以最保险的作法是: 在信号处理程序中, 自己处理所有的流

<br/>


### system问题引导(`回顾第8章`)
- 在第8章简单学习过system函数, 它的原理是`few`机制, 先来回顾一下当时自己实现的`my_system`
...
> 上述程序在原来的基础上稍做了部分修改. 
>   1. `execl(cmd)`的调用被改为了直接传递一整个字符串参数
>   2. `_exit(-1)`的参数直接改为了-1, 表示execl失败
>
> <br/>
>
> 现在来讨论为什么要在`system`中对信号做处理
> 首先要清楚一点的是: <font color=red>system是为了将任务交给其他进程处理, 而调用它的进程要等待另外的进程结束, 从逻辑上来讲流程图应该是这样的</font>

```mermaid
sequenceDiagram
    participant main
    participant ed
    participant newshell
    main->>ed: system ed
    Note right of ed: ed进程执行中
    newshell->>ed: kill SIGTERM
    ed-->>main: kill SIGCHLD, system内部处理
    Note right of main: system返回,继续往下执行
```

<br/>


> 其中虚线箭头表示的是ed会产生该信号, 但调用进程main用户代码不处理, 由system内部处理
> 上面这个图的顺序就是system的流程, 调用system的进程会被system所阻塞, 等到system所创建的子进程结束后, 调用进程会从system返回. <font color=red>所以这一点在实现上应该是 system的子进程异常结束, 调用进程也不应该结束,并且调用进程不需要知道具体子进程的状态, 应该由system的返回值来处理</font>. 简单来说如下:

```txt
    1. 调用进程 main 不应该被结束. 准确来说是不能在运行system期间被结束
        
    2. 调用进程 main 不应该处理system所创建的子进程状态

第1点原因:
    main调用 system一直到 ed, 大致的流程是
        main ~~> system ~~> fork: 
                                 ==0: child ~~> exec("ed") ==> ed
    ...
    此时 main进程和 ed进程都是前台进程
    若发送一个 SIGINT, 则:
        main会直接被结束
        ed由于自身已经阻塞了SIGINT, 不会结束
    这种情况下 已经不符合上面这幅图的流程, main居然被结束了
    正常情况下 main进程应该是像后台作业一样, 等待 ed进程结束


第2点原因:
    main进程如果 拦截SIGCHLD并在信号处理中wait, 则system所创建的ed进程被异常或正常结束时, 都会以
    这样的顺序被处理:
        1. main进程中信号处理中wait先捕捉到ed的终止状态
        2. main进程中信号处理完毕后, system内部的wait才会响应,
            这个时候因为ed进程已经在信号处理wait处理了, 所以这次会失败,返回-1

    所以 system返回-1的这种情况并不是 exec失败了, 综上:
        调用进程main不应该拦截SIGCHLD信号, 更不能在信号中做wait等待



system:
    在第8章实现的my_system中缺少的信号处理就是这2个原因, 系统的system
    内部的实现已经对上述2个原因作了有效处理
```

<br/>

> 在完善`my_system`之前, 先做2个测试(`以小节形式展开`):
>   1. 测试`my_system`存在的问题1和问题2
>   2. 对于问题2所返回的-1, 是不是真的因为2次wait的调用所导致system这1次失败
> 最后解决这2个问题, 完善`my_system`
...
> main进程可能直接结束
>
> 使用`my_system`来测试`ed`, 要从2方面来测试:
>   1. ed运行起来后, 查看当前终端的前台进程, main是不是前台
>   2. 发送`SIGINT`后, 查看main和ed这2个进程的状态
>
> <br/>
>
> 省略`my_system`的实现, 这里直接给出main的测试程序
...
./main
pid[5111]                   # __shell_1
a                           # __shell_4 此时交互的是ed进程[5111]
hello world                 # __shell_5 
.                           # __shell_6
1,$p                        # __shell_7
hello world                 # __shell_8
stdin: Input/output error   # __shell_10
...
## 辅助查看的shell
ps -o pid,ppid,comm,state | grep -e "main" -e "ed"      # __shell_2
5111  2419 ./main S+                                    # __shell_3
5112  5111 (ed)   S+                                        

kill -s INT 5111                                        # __shell_9
ps -o pid,ppid,comm,state | grep -e "main" -e "ed"      # __shell_11
5112     1 (ed) S                                       # __shelL_12
kill 5112                                               # __shell_13
```

<br/>

> 上述程序的测试流程如`__shell_N`所示
> 1. `__shell_1`: 进程运行后`main`进程的`pid`
>   - 此时进程已经启动了ed进程, 并阻塞等待用户输入
> 2. `__shell_2`: 在另一终端查看当前进程的状态
> 3. `__shell_3`: 由第2步中, 查看可以看到:
>   - main处于阻塞(<font color=green>睡眠</font>), 并处理前台`__shell_3`
>   - 同时`ed`也是阻塞并处于前台进程
> 4. `__shell_4`: 用户输入`a`, 表示告诉ed追加内容
> 5. `__shell_5`: 用户输入追加的内容 <font color=red>hello world</font>
> 6. `__shell_6`: 用户输入`.`, 表示告诉ed结束这1次的输入
> 7. `__shell_7`: 用户输入`1,$p`, 表示告诉ed查看第1次的输入
> 8. `__shell_8`: ed打印出第1次输入的内容
> 9. `__shell_9`: 向`main`发送`SIGINT`信号
>   - 因为main并未处理`SIGINT`, 所以直接被结束. 当main被结束后, 它的父进程`shell`会响应作出处理, 在作业控制下 <font color=red>shell会切换前后台进程</font>. 具体的作法是直接调用`tcsetpgrp`将`自己设置为前台进程`, 此时对应的其他所有的进程会被强制变成后台进程. 
> 10. `__shell_10`: 由ed进程所打印的错误. 
>   - 当第9步后, ed被切换为后台进程, 但ed本身是阻塞等待用户输入命令的, 换句话说: <font color=red>ed内部是循环调用read而产生阻塞</font>. 所以当后台进程ed向终端read时, 终端驱动会发送`SIGTTIN`到`ed`, 造成ed的read报错, 同时ed打印出报错的原因`__shell_10`. 又因为ed循环, 所以又再次read终端, 形成循环打印`__shell_10`
> 11. `__shell_11`: 在另一终端查看当前ed的状态
> 12. `__shell_12`: 当第11步查看ed时, ps此刻获取到的ed状态是睡眠(`阻塞`), 可以发现, 它确实是一个后台进程
> 13. `__shell_13`: 直接kill掉ed进程
> <br/>
> 下面的gif图是重新运行进程的, 对应的进程id不一样, 但测试的步骤是一样的
...
### `my_system`问题2演示
> <font color=deeppink size = 5>测试2--SIGCHLD拦截导致wait失败</font>
>
> 在这里要提一下`__code_warning_0`, 它的意义是: <font color=red>阻塞的系统调用可能其他被信号中断, 这种情况下并不是一个真正的错误, 而是一个假中断, 在后续的socket中这种处理很常见. 整体上的语义是若是EINTR假中断, 则继续循环</font>
...
int main(int args, char** argv){
    ...
    sigemptyset(&st.sa_mask);
    st.sa_sigaction = [](int no, siginfo_t* info, void* ctx){
        if(no == SIGCHLD){
            cout << "over:\t" << info->si_pid << endl;
...
> 如上述程序: <font color=red>main拦截SIGCHLD并调用wait, 这样就会导致`my_system`所产生的ed进程在结束时在信号函数中被处理, 造成`my_system`函数内部wait调用失败</font>. 测试步骤:
> 1. 运行程序
> 2. 在另一shell中对ed发送TERM信号
> 3. 观察打印结果
...
child[7039]
over:	7039                                # __shell_3
wait: Interrupted system call               # __shell_4
wait: No child processes                    # __shell_5
res:	-1


### 在另一shell对ed[7039]发送TERM
kill 7039                                   # __shell_2
```
...
> 测试的过程由`__shell_N`指示:
> 1. `__shell_1`: 运行程序, 然后输出各自的进程id
> 2. `__shell_2`: 在另一终端中发送`SIGTERM`到`ed[7039]`
> 3. 第2步操作完后, main接收到信号, 并打印相关的信息`__shell_3`, 之后立即调用了`waitpid`处理了ed进程
> 4. 在`my_system`中的`waitpid`会被`SIGCHLD`的信号假中断打断, 所以`__shell_4`先被执行
> 5. 但`my_system`是循环处理, 所以又调用`waitpid`造成`__shell_5`打印
> 
> <br/>
> 
> 测试图如下
...
<img src="assets/10_28.gif"/>


<br/>

### 错误的waitpid返回处理
- 在第8章学习`waitpid`时只是简单的对该函数的返回值作了简短的说明, 前面测试中的`__code_warning_0`严格来说是错误的判断逻辑. 根据waitpid的返回值, `my_system`应该是这样来wait的(<font color=green>伪代码</font>)
...
先来回顾waitpid返回值的情况
    ...
    4. 若在阻塞期间被其他信号打断则返回-1并且errno为EINTR
int my_system(cmd){
...
### 使用`waitid扩展下的WNOWAIT`选项
- 在前2个小节中, 在main程序中拦截了`SIGCHLD`所造成的后果是`my_system`内部waitpid失败. 有2种方案解决:
    1. `my_system`内部屏蔽`SIGCHLD`
    2. main拦截SIGCHLD, 但要使用`WNOWAIT`选项
> 下面的测试改写了`my_system`的wait处理
...
_begin:
    ...
    ret = waitpid(pid, &sta, 0);            // __code_handler

    // 业务错误(当前进程前后没有任何要等待的子进程)
    if(ret < 0 && errno == ECHILD){
        cerr << "wait-nochld[" << pid << "]\t" << strerror(errno) << endl;
        return -1;
    }

    if(ret < 0){
       cerr << "EINTR[" << EINTR << "]\terrno[" << errno << "]\n";
       cerr << "wait-error[" << pid << "]\t" << strerror(errno) << endl;
       if(errno == EINTR)
        goto _begin;
       return -1;
    }
...
int main(int args, char** argv){
    ...
    st.sa_sigaction = [](int no, siginfo_t* info, void* ctx){
        if(no == SIGCHLD){
            cout << "over:\t" << info->si_pid << endl;
            waitid(P_PID, info->si_pid, nullptr, WNOWAIT | WEXITED);
            return;
        }
    };
...
> 上述测试程序使用了`waitid`的标准扩展, 指定了`WNOWAIT`. 它可以取出子进程的终止状态来查看一下, 并不会对子进程作善后处理, 真正处理的地方是`__code_handler`. 整个测试步骤:
>   1. 运行进程
>   2. 查看进程的状态
>   3. 杀掉ed进程
>   4. 观察打印
...
./main                                          # __shell_1
pid[4024]                                       # __shell_2
child[4025]                                     # __shell_3
over:	4025                                    # __shell_8
EINTR[4]	errno[4]                            # __shell_9
wait-error[4025]	Interrupted system call     # __shell_10
res:	15                                      # __shell_11



## 另一个shell中
ps -o pid,ppid,comm,state | grep -e "main" -e "ed" # __shell_4
4024   570 ./main S+                               # __shell_5
4025  4024 (ed)   S+                               # __shell_6
kill 4025  # 杀死ed                                # __shell_7
```

---


<img src="assets/10_29.gif"/>


<br/>

> 在这里`__code_handler`被调用了2次:
>   1. <font color=red>被中断打断</font>(`__shell_10`)
>   2. 中断打断后循环, 直接处理了返回(`main函数中__shell_11处被打印`)
...
### 完善`my_system`
- 根据前面的总结和测试, `my_system`必须处理信号问题, 所以`my_system`还需要:
    1. 屏蔽 <font color=deeppink>SIGINT</font>
    2. 屏蔽<font color=deeppink>SIGCHLD</font>
    3. 最后wait结束后, 还原信号
...
int my_system(const char* cmd){
    ...
    sigset_t set,oset;
    sigprocmask(0, &oset,nullptr);
    sigemptyset(&set);
    sigaddset(&set, SIGINT);
    sigaddset(&set, SIGCHLD);

    sigprocmask(SIG_BLOCK, &set,nullptr);       // 阻塞 SIGINT和SIGCHLD

    int sta;
    pid_t ret;

    while(1){
        ret = waitpid(pid, &sta, 0);
    ...
    sigprocmask(SIG_SETMASK, &oset,nullptr);    // 还原信号
    return sta;
}
```
...
- 它的返回值可以从它内部的结构来制定:
    ...
    2. `execl`调用是否成功
    3. `waitpid`的调用返回
> 现在来一个一个说明这些情况的返回, 并制定相关的返回策略给外界

```txt
fork情况:
    成功时 my_system不返回
    失败时 my_system返回-1, 此时errno会有对应的错误编号



execl情况:
    若成功, execl本身并不返回, 导致 _exit(-1)不会执行
        若execl后的子进程 正常返回, 则my_system的wait正确处理得到 sta, 返回到用户
        若.........       异常.....       ... ..        ...       sta, 返回到用户

    若失败, _exit(-1)返回, 此时errno也会被设置, 子进程主动调用 exit, 也会由wait处理得到 sta返回


waitpid情况:
    1. 正确处理得到 sta返回
    2. 本身调用失败 返回-1, 并设置errno


    PS: 我们可以将这所有的情况制定成不冲突的值返回给外界, 但事实上
        系统下的system库函数对返回值是统一作处理:
            -1: 调用fork或waitpid失败
            127:exec调用失败
            其他: exec后的进程 正常返回(0)或异常返回(主动exit(code)或某个信号杀死)

    综上: my_system的返回基本不修改, 只需要将 _exit(-1)改为 _exit(127)就够了
            其中127表示exec调用失败, 但有一种情况是用户在子进程中直接  exit(127)时也会有这种情况
```

<br/>

> 将上面的`my_system`稍作修改来测试一下<font color = red>子进程异常返回时返回值是不是信号的编号</font>
...
pid[6083]
child[6084]
res:	15                                  # 返回15, 刚好是信号SIGTERM
err-res	: Invalid argument

lb ./main
pid[6159]
child[6160]
res:	11                                  # 返回11, 刚好是信号SIGEV
err-res	: Invalid argument
...
## 另一个shell中
kill -s TERM 6084                           # SIGTERM编号是15
kill -s URG 6084
kill: kill 6084 failed: no such process
kill -s URG 6160                            # SIGURG并不结束子进程,所以上面没有反应
kill -s SEGV 6160                           # SEGV编号是11
```
...
### 系统system
- 它的实现好上述的`my_system`, 使用时一定要查阅手册明白它的返回值
...
### sleep版本4(`缺陷`)
- 在前面(如[版本3](#sleep-version-3))实现的`my_sleep`中始终不能确保 <font color=deeppink>在调用pause前是不是已经处理了alarm</font>, 这一部分应该用信号来处理:
    - <font color=deeppink>即在pause前不允许发生alarm</font>
...
sigjmp_buf func_stack;

static auto _alarm_h = [](int no) -> void {
    siglongjmp(func_stack, 1);
};
...
static inline int my_sleep(int time_s){
   ...
   // sleep期间 SIGALRM临时的信号设置信息
    struct sigaction tmp_alarm_h;

    // 保存最开始的SIGALRM设置信息
    struct sigaction old_alarm_h;

    // 这4步 设置SIGALRM指向 _alarm_h
    sigemptyset(&tmp_alarm_h.sa_mask);
    tmp_alarm_h.sa_flags = 0;
    tmp_alarm_h.sa_handler = _alarm_h;
    if(sigaction(SIGALRM, &tmp_alarm_h, &old_alarm_h) < 0){ // __code_register_alrm
        return time_s;
    }
    ...
    // 在调用 alarm前 阻塞SIGALRM, 这里要区分上面 __code_register_alrm
    // 这里是保证在后面出现 alarm调用前不产生 alarm信号
    sigemptyset(&bset);
    ...
    if(sigsetjmp(func_stack,1) == 0){
        ...
        // 主业务调用完 alarm后, 先解除SIGALRM阻塞, 调用sigsuspend
        // 这样 alarm和sigsuspend之间一定不会产生alarm信号
        tmp_set = oset;
        sigdelset(&tmp_set, SIGALRM);
        sigsuspend(&tmp_set);                           // __code_pause
    }else{
        alarm(ret);                                     // __code_0
    }

    // 先还原之前 SIGALRM的设置
    sigaction(SIGALRM, &old_alarm_h,nullptr);
...
> 上述程序总体上来说就是: <font color=red>进入函数前改变SIGALRM的设置, 然后在调用alarm前阻塞SIGALRM信号, 配合sigsuspend解除SIGALRM并暂停的原子过程可以实现sleep函数中对信号的处理</font>. 需要说明的是, 在最后还原时要先还原SIGALRM的设置, 再还原信号. 因为若是先还原信号, 则可能会出现sigprocmask处理未决信号的问题, 这个时候信号处理函数还指向`_alarm_h`. 该版本 <font color=red>虽然将longjmp修改为了siglongjmp, 但它还是存在结束其他未返回的信号处理程序的问题</font>


<br/>

### 系统的sleep
- 事实上前面实现sleep的版本始终和alarm牵扯, 所以不管采用什么机制一定会和alarm冲突. 然而sleep本身是一个<font color = deeppink>库函数</font>. 不同Unix平台在实现底层的系统调用时并不是采用`alarm-pause`. 而是另一种方案, 和`alarm`完全独立开来. 相关的函数如下:

```cpp
#include<unistd.h>
int sleep(int sec);

// sleep虽然头文件是 标准Unix, 但事实上它是标准ISO C所定义
// 调用该函数可能:
///// 阻塞 sec 秒
///// 在sec秒到来前, 被信号打断, 此时返回 距离sec的秒数


#include<time.h>
int nanosleep(struct timespec* reqtp, struct timespec* remtp);
/// 该函数由IOS C定义, 由操作系统实现,timespec可以精确到纳秒, 但不同的平台上精度不一样, 如mac上是微秒, linux上是纳秒
/// 返回0, 表示睡眠时间到. 出错返回-1


int clock_nanosleep(clockid_t clock_id int flags, struct timespec* reqtp, struct timespec* remtp);
// 同nanosleep, 但该函数可以指定时钟, 并通过flags来指定 绝对时间和相对时间
```

<br/>

> 系统实现sleep基本是通过nanosleep, 后面2个函数是和信号独立开的, 所以不用关心和信号处理的交互

<br/>

### nanosleep
- 标准C定义了接口, 要求最大精度达到纳秒, 但这是根据实现决定.
    - 在Mac平台上`clock_getres`测试的精度是微秒, linux上则是纳秒(<font color = green>具体见第6章 获取不同时钟精度</font>)
- 在里需要对参数作出说明
    1. `reqtp`设置相对`此刻时间`未来的间距时间, 最大精确到纳秒
    2. `remtp`若在睡眠中被打断, 则将剩余时间填充到该参数中
...
int main(int args, char** argv){
    ...
    bool is_nsec = now.tv_nsec == 1;

    cout << "支持纳秒:\t" << (is_nsec ? "yes" : "no") << endl;

    clock_gettime(CLOCK_REALTIME, &now);
    cout << "sec:\t" << now.tv_sec << "\tnsec:\t" << now.tv_nsec << endl;

    string date("/bin/date -r ");
    date += to_string(now.tv_sec);

    system(date.c_str());

    now.tv_nsec = 0;
    now.tv_sec = 5;
    nanosleep(&now,nullptr);                // 使用时 now 相关的部分要清零一下
...
./main
支持纳秒:	no
sec:	1690702607	nsec:	334117000
2023年 7月30日 星期日 15时36分47秒 CST
2023年 7月30日 星期日 15时36分52秒 CST
```
...
### `clock_nanosleep`
- 和`nanosleep`不同的是, 该函数可以对指定的时钟睡眠. 遗憾的是Mac上并未提供该函数下面以linux来演示
...
int main(int args, char** argv){
    ...
    if(!atoi(argv[1])){
    	now.tv_nsec = 0;
    	now.tv_sec = 5;
    	clock_nanosleep(CLOCK_REALTIME, 0, &now, nullptr);
    }else{
    	now.tv_sec += 5;
    	clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &now, nullptr);
    }
...
> `clock_nanosleep`的第2个参数只有2个值: <font color=red>0和1</font>(1[`TIMER_ABSTIME`]). 1表示的是相对时间, 所以在设置时以加上相对点的时间
...
./main 1
支持纳秒:	yes
sec:	1690704399	nsec:	916060252
2023年 07月 30日 星期日 16:06:39 CST
2023年 07月 30日 星期日 16:06:44 CST
liubo-linux% ./main 0
支持纳秒:	yes
sec:	1690704408	nsec:	113553534
2023年 07月 30日 星期日 16:06:48 CST
2023年 07月 30日 星期日 16:06:53 CST
```

<br/>

### 纳秒其实不准确
- 虽然在linux上睡眠的精度在1纳秒, 但事实产生的时钟中断不会达到这个精度. 可以简单分析一下, 常规计算机 <font color=red>时钟滴答周期精度在毫秒</font>. 同时常规计算机指令周期也在1个纳秒左右, 再加上调度切换等各种因素, 都会导致结果达不到纳秒的精度. 
...
#define MNS 1000000ull

#define CL_ID CLOCK_REALTIME

#ifdef __APPLE__
    #define CL_FUN nanosleep
#elif defined __linux__
    #define CL_FUN(args...) clock_nanosleep(CL_ID, 0, args)
#else
    #define CL_FUN nanosleep
#endif
...
int main(int args, char** argv){
    if(args < 3){
        cerr << "usage main <0:1个时钟中断周期, 1:1纳秒> no\n";
        return -1;
    }
    int type = atoi(argv[1]);                           // __code_0


    ostringstream str;

    auto dida = sysconf(_SC_CLK_TCK);                   // __code_1
    str << "----------------------------------" << argv[2] << "------------------------\n";
    str << "dida:\t" << dida << endl;                   // __code_2

    struct timespec c = {0}, s, e;
    c.tv_sec = 0;
    // 时钟中断数 * 100,0000纳秒                    
    c.tv_nsec = type == 0 ? 1000 / dida * 1000000 : 1;  // __code_3

    str << "total time:\t" << static_cast<double>(c.tv_nsec) / MNS << "毫秒\n";


    clock_gettime(CL_ID,&s);                            // __code_sleep_before
    CL_FUN(&c, nullptr);                                // __code_sleep_ing
    clock_gettime(CL_ID,&e);                            // __code_sleep_after
    str << "begin:\t" << s.tv_sec << "\t" << s.tv_nsec << endl;
    str << "end:\t" << e.tv_sec << "\t" << e.tv_nsec << endl;
    str << "time:\t" << (static_cast<double>(e.tv_nsec) - s.tv_nsec) / MNS << "毫秒\n";
    str << "----------------------------------" << argv[2] << "------------------------\n\n";
    cout << str.str();
    return 0;
}
...
> <font color=red>上述程序分别要在linux和mac上进行测试</font>. 从命令行接收2个参数:
>   1. `type`表示时间的精度测试: `0`[<font color=red>当前系统时钟周期</font>]; `1`[<font color=red>1纳秒</font>]
>   2. `no`: 用来区分打印序列
> 
> <br/>
>
> 测试流程:
> 1. 程序运行后, 获取当前系统的 <font color=red>时钟滴答数</font>(`__code_1`), 这里在Mac和Linux上都是10毫秒产生一次时钟中断
>   - 在不同的平台上不一样, 可以去看第6章系统时间
> 2. 根据type的测试类型决定睡眠时间(`__code_3`)
> 3. 在睡眠前统计当前的系统时钟(`__code_sleep_before`)
> 4. 调用睡眠, <font color=red>在不同的平台可能调用的不一样</font>
> 5. 在睡眠后统计当前的系统时钟(`__code_sleep_after`)
> 6. 计算相关的时间并转化输出到控制台
>
> <br/>
>
...
```shell
#### Mac, 时钟周期10毫秒测试
for i in {1..5}; do {./main 0 $i} done;
----------------------------------1------------------------
dida:	100
total time:	10毫秒
begin:	1690784240	857205000
end:	1690784240	869729000
time:	12.524毫秒
----------------------------------1------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784240	883704000
end:	1690784240	896216000
time:	12.512毫秒
----------------------------------2------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784240	905362000
end:	1690784240	917891000
time:	12.529毫秒
----------------------------------3------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784240	926648000
end:	1690784240	939177000
time:	12.529毫秒
----------------------------------4------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784240	948114000
end:	1690784240	960633000
time:	12.519毫秒
----------------------------------5------------------------

#### mac上指定10毫秒时, 会有近2毫秒的误差, 当然并未看具体的用户代码的时间
...
dida:	100
total time:	1e-06毫秒
begin:	1690785059	169612000
end:	1690785059	169631000
time:	0.019毫秒
----------------------------------1------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785059	179718000
end:	1690785059	179734000
time:	0.016毫秒
----------------------------------2------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785059	187784000
end:	1690785059	187799000
time:	0.015毫秒
----------------------------------3------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785059	195349000
end:	1690785059	195364000
time:	0.015毫秒
----------------------------------4------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785059	203538000
end:	1690785059	203552000
time:	0.014毫秒
----------------------------------5------------------------


#### 当Mac指定1纳秒时, 根据前面的分析, 系统不会以1纳秒准确中断, 睡眠的时间近 15微秒
...
### Linux(Ubuntu) 时钟周期10毫秒 测试
liubo-linux% for i in {1..5}; do { ./main 0 $i} done;
----------------------------------1------------------------
dida:	100
total time:	10毫秒
begin:	1690784234	276215892
end:	1690784234	286600619
time:	10.3847毫秒
----------------------------------1------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784234	293840594
end:	1690784234	304010359
time:	10.1698毫秒
----------------------------------2------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784234	313030150
end:	1690784234	323504503
time:	10.4744毫秒
----------------------------------3------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784234	331523822
end:	1690784234	341795088
time:	10.2713毫秒
----------------------------------4------------------------
...
dida:	100
total time:	10毫秒
begin:	1690784234	349684114
end:	1690784234	360193676
time:	10.5096毫秒
----------------------------------5------------------------

#### 相对的Linux中指定10毫秒时, 睡眠的时间误差比Mac小, 但前面说了并未准确去测试Mac睡眠的用户代码时间(`__code_sleep_ing`)
...
dida:	100
total time:	1e-06毫秒
begin:	1690785138	903277459
end:	1690785138	903386870
time:	0.109411毫秒
----------------------------------1------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785138	907410174
end:	1690785138	907512378
time:	0.102204毫秒
----------------------------------2------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785138	911017916
end:	1690785138	911107453
time:	0.089537毫秒
----------------------------------3------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785138	914312839
end:	1690785138	914419209
time:	0.10637毫秒
----------------------------------4------------------------
...
dida:	100
total time:	1e-06毫秒
begin:	1690785138	917297360
end:	1690785138	917377148
time:	0.079788毫秒
----------------------------------5------------------------


#### Linux指定1纳秒时, 睡眠的时间 大致 80微秒, 误差比Mac大
```

<br/>

> 结论:
>   1. 当指定1个时钟滴答周期(<font color=green>这里是10毫秒</font>)时, 由于此刻系统各种复杂因素, 并不会准确的阻塞10毫秒
>   2. 当指定时间 <font color=red>小于1个时钟滴答周期</font>(<font color=green>这里是纳秒</font>)时, 更不可能达到这个精度. 又因为滴答周期是10毫秒, 其2个平台产生的结果又远小于10毫秒, 可以推断出, 内核处理nanosleep时, 并不是以滴答周期实现. 猜想应该是更精确的时间机制


<br/>

### unix中周期定时器
- 前面用`alarm-pause`实现的[`alarm多任务定时器`](#mul-task-timer)存在比较多的问题:
    1. 精度问题
    2. 信号处理(`alarm和pause之间要处理很多代码`)

- 可以使用系统提供的周期定时器来解决上面的问题
...
> 函数中的时间信息是用`struct itimerval`表示. 在第6章已经学习过了`struct timeval`, 这里再次看一下整体的结构
...
> 整个`setitimer`的含义是: <font color=red>设置`which`类型的定时器, 在时间点`it_value`开始计时,每隔`it_interval`后重复计时</font>. 这里要着重介绍参数`val`.
...
这里设置时间的最小值要根据当前系统的 滴答数
如Mac上滴答数是 100, 则表示1秒时钟滴答100次, 即每10毫秒产生一次时钟中断
所以设置的时间不能小于 10毫秒
```

<br/>

- 参数`which`表示定时器的类型, 有3种

|类型|描述|
|:-:|:-|
|`ITIMER_REAL`|现实世界的时间, 也可以看成当前系统时间,超时产生`SIGALRM`信号|
|`ITIMER_VIRTUAL`|进程运行时才计时(<font color = green>用户态</font>,超时产生`SIGVTALRM`信号|
|`ITIMER_PROF`|进程在运行时才计时(<font color=green>用户态和内核态</font>),超时产生`SIGPROF`信号|


<br/>

> 在必要对这3种类型的计时作说明:
> <font color=red>真实时间最容易理解, 即设置多长时间, 只要进程未结束, 不管它是不是阻塞暂停还是运行的都计时. 其余的2种定时器只有进程在运行时才计时, 即阻塞暂停时不计</font>. 下面以1个简单的程序来测试
...
static void show_time(int no){
    struct timespec s;
    clock_gettime(CLOCK_REALTIME, &s);
    cout << "time:\t" << s.tv_sec << "\t" << s.tv_nsec << endl;
}

static void init_sig(int no){
    signal(no, show_time);
}

static void do_timer(decltype(ITIMER_REAL) type){
    struct itimerval val = {0};
    val.it_interval.tv_sec = TIME_INTERVAL;
    val.it_value.tv_sec = TIME_BEGIN;
    setitimer(type, &val, nullptr);
}

int main(int args, char** argv){
    if(args < 2){
        cerr << "usage main <timer-type 1:[REAL], 2:[VIRTUAL], 3:[PROF]> <block 0:非阻塞 1:阻塞>\n";
        return -1;
    }

    typedef decltype(ITIMER_PROF)   timer_t;
    typedef decltype(SIGALRM)       sig_t;
    using Timer = tuple<const char*, timer_t, sig_t, const char*>;
    vector<Timer> timers;
    timers.push_back(make_tuple("1", ITIMER_REAL,       SIGALRM,        "ITIMER_REAL[真实时间]"));
    timers.push_back(make_tuple("2", ITIMER_VIRTUAL,    SIGVTALRM,      "ITIMER_VIRTUAL[虚拟时间]"));
    timers.push_back(make_tuple("3", ITIMER_PROF,       SIGPROF,        "ITIMER_PROF[运行时间]"));

    for_each(timers.begin(), timers.end(), [tmp_argv=argv](Timer& item){
        if(!strncmp(get<0>(item), tmp_argv[1], strlen(tmp_argv[1]))){
...
### 排队信号
- <font color=deeppink>前面所有的信号统称为标准信号, 共31个</font>. 由于最开始架构的机制, 会导致<font color = deeppink>信号的不可靠</font>. 在后续的标准扩展中增加了时时信号(`也称为排队信号`). 固名思义就是对信号进行排队, 所以这种信号的结构和原来的标准信号是不一样的.

- 信号不允许无限排队, 因为会有系统限制(`SIGQUEUE_MAX`). 时时信号属于信号扩展不在标准中, 它的范围在`SIGRTMIN ~ SIGRTMAX`闭合区间内
...
int main(int args, char** argv){
	...
	cout << sysconf(_SC_SIGQUEUE_MAX) << endl;
#endif
	cout << SIGRTMIN << endl;
	cout << SIGRTMAX << endl;
	return 0;
...
> 上述程序在linux上测试进程中信号排队的最大数量

<br/>

- 并不是所有的平台都有时时信号, 因为时时信号只是扩展, 并不是标准, 所以会有所不同
...
> 所以在Mac上没有时时信号, 这也是`sizeof(siginfo_t)`在mac上是32个bit位的原因. 在linux中则是128位.

<br/>

- 时时信号允许用户在发送时由内核附加更多的信息, 这些信息通过`siginfo`结构传递到信号处理. 时时信号必须通过`sigqueue`函数来发送, 发送时可以自己指定信息
```cpp
#include<signal.h>
...
> 该函数只能将信号发送到单个进程, 可以通过value传递信息, value在sigaction中已经介绍
> 使用步骤:
>   1. 使用`sigaction`指定`SA_SIGINFO`
>   2. 通过`sigqueue`发送信号
...
int main(int args, char** argv){
	...
	ac.sa_sigaction = [](int no, siginfo_t* info, void* msg){
		cout << info->si_value.sival_int << endl;
		if(info->si_value.sival_int == 1)
			sleep(10);
	};
	sigaction(SIGRTMIN + 1, &ac, nullptr);
...
> 上述程序流程:
>   1. 运行程序后, 子进程会周期发送`SIGRTMIN + 1`这个信号, 附加计数`count`
>   2. main进程通过注册`SIGRTMIN + 1`, 打印接收到的计数
> 需要说明一点的是: <font color=red>子进程在5秒内发送5次信号, main进程中第1次接收后会睡眠10秒</font>, 最后测试的结果是main进程10秒瞬间处理所有排队信号
...
|信号|说明|
|:-:|:-|
|SIGCHLD|子进程暂停(`停止`)或终止|
|SIGCONT|子进程从暂停然后继续运行|
|SIGSTOP|终止,不能被捕捉或忽略|
|SIGTSTP|前台程序暂停(`Ctrl+Z`)|
|SIGTTIN|后台进程组成员读控制终端|
|SIGTTOU|后台进程组成员写控制终端|

> 一般情况下不处理这些信号(<font color=green>SIGCHLD除外</font>). 但交互式程序会处理它们(<font color=green>如shell</font>). 
> 另外其他一些需要管理终端的进程(<font color=green>如vim编辑器</font>). 它也会处理这些信号. 例如: <font color=red>当vim启动时, 它会接管控制终端, 并重新绘制终端窗口. 当要对vim暂停时, 它会处理`Ctrl+Z`信号, 重新编制终端, 将终端恢复到启动vim时的画面</font>. 类似的还有top进程, 它每隔一定时间都重新绘制终端窗口, 对top进程暂停时, 它也会处理该信号, 重新绘制终端恢复启动top时的画面
...
- 要理解它, 就要从2个层面来看待:
    ...
    2. 配合进程暂停
> 但它也有一些其他特点:
>   - 该信号不做任何设置时, 信号处理函数是`SIG_DFL`, 此时它的效果是: <font color=red>进程运行期间收到该信号不会被终止; 进程被暂停时收到该信号会继续运行; 该信号不打断阻塞状态</font>
>
> 注意进程暂停(<font color=green>停止</font>)和挂起(<font color=green>如pause</font>)不同

<br/>

> <font color=red size = 5>测试进程运行间不被终止</font>
...
int main(int args, char** argv){
    ...
    // 查看该信号的系统配置
    auto previous = signal(SIGCONT, [](int no){});
    cout << "default action:\n\t\tignore[" << (previous == SIG_IGN) << "]\t";
    cout << "default[" << (previous == SIG_DFL) << "]\n";

    // 还原到系统的配置
    signal(SIGCONT, previous);

    // 进程运行期间发送该信号
    kill(getpid(), SIGCONT);

    // 来到这里表明该信号不终止进程
    cout << "post sigcont over, will pause\n";
...
./main
default action:
		ignore[0]	default[1]      # 系统默认的是 SIG_DFL
post sigcont over, will pause       # 当进程中kill后, 并未被终止
...
> <font color=red size = 5>测试进程阻塞时不可以被继续</font>
...
```shell
./main
will post sigcont           # main发送 SIGCONT, 此时child已经是 S+, 属于阻塞状态
...
> <font color=red size = 5>测试进程暂停时可以被继续</font>
...
int main(int args, char** argv){
    ...
    system("ps -o pid,ppid,comm,state");
...
```shell
./main
  PID  PPID COMM   STAT             # 过1秒时, 输出当前会话中所有进程的状态, child为 T+, 表示被前台被暂停
  754   753 -zsh   S
 3932   754 ./main S+
 3934  3932 ./main T+
 1555  1554 -zsh   S+
will post sigcont                   # 对child发送SIGCONT
over                                # child被继续后, 打印再结束
```

<br/>

> <font color = red>这里因为是在child自己在代码中主动发送`SIGTSTP`, 在正常的shell作业中, 应该是用户摁下`Ctrl+Z`, 由驱动对当前前台进程发送`SIGTSTP`, 内核处理这个信号, 将该进程暂停, shell收到子进程状态变化, 再切换该进程到后台进程</font>. 这里为了演示方便测试直接用代码发该信号

<br/>

> <font color=red size = 5>即使SIGCONT被忽略或阻塞, 则在产生信号时, 它的功能还是保持不变</font>
...
int main(int args, char** argv){
    ...
    if(child == 0){
#if 0
        signal(SIGCONT, SIG_IGN);       // __code_0
#else
        sigset_t s;
        sigemptyset(&s);
        sigaddset(&s, SIGCONT);
        sigprocmask(SIG_BLOCK, &s, nullptr);    // __code_1
#endif
        kill(getpid(), SIGTSTP);
        cout << "over\n";
        return 0;
...
```
> 上述测试中: <font color=red>不管对SIGCONT是忽略(`__code_0`)或阻塞(`__code_1`), 子进程都能收到该信号, 并从T状态到R状态</font>
...
### 信号名和编号
- 前面所用到的所有信号上都是1个整数, 事实上它们都有自己的字符串名, 相关的函数如下:
...
// 类似于 perror, 一般错误时, 将信号错误信息输出到 标准错误

void psiginfo(const siginfo_t* info, const char* msg);
// 使用sigaction.sa_sigaction回调时, 打印siginfo_t相关的信息, 不同的系统可能描述不一样
// msg是类似于perror的前缀
...
### 总结
- 信号是Unix中最重要的一章之一. 本章花了大量的精力去细读书籍, 并且对每1个知识点都做了详细的测试. Unix中最终的目的是系统编程, 此章是后续终端IO的关键. Unix中的应用类软件就是终端IO, 如优秀的程序Vim等, 只有了解Unix运行机制, 我们也可以通过终端IO作出Unix下的终端软件, 这比所谓的窗口程序更加有趣, 更细节. 信号中还有很多扩展知识, 这些都需要更多的精力去学习. 

...
> 上述代码用宏区分当前环境是多线程还是多进程:
>   1. 多线程时使用了C++标准库中的mutex(<font color=green>在Unix环境下会调用到POSIX底层pthread相关的互斥锁</font>).
>   2. 多进程使用了文件锁. 
> 
> 在临界区(`LOCK与UN_LOCK`)所要做的是从文件中读取内容做加1操作后写入到文件, 其中特意睡眠了1秒. 从测试打印中可以发现, 当解锁后只有1条线程或进程被唤醒. 这当然是符合常理的.
>
> 在用户层面所观察到的是只有1条线程被唤醒了, 所以以笔者的定义认为是非惊群的
>   - 不能从这一点来推断出内部不是惊群的, 因为可能唤醒了所有的进程, 然后只有1条进程抢到了锁, 其他进程则抢到锁失败导致又睡眠(`笔者没有去查看相应的文献, 也没有去观察源码`).
>   - 对于互斥锁来说, 在Linux下一定不是惊群的. 在第`11_12`章线程的章节中, 通过源码了解到glibc中的互斥锁内部调用了`futex_wait(Linux)`, 当调用`pthread_mutex_unlock`或`pthread_cond_signal`时只会唤醒一条线程. 当调用`pthread_cond_broadcast`时会唤醒多条线程, 这一点在手册中已经说明. 
>
> PS: `pthread_cond_broadcast`内容虽然唤醒了多条线程, 但事实上因为内部实现机制, 醒来的线程又会竞争调用到`pthread_mutex_lock`, 没有抢到锁的又会挂起. 所以在用户层面来看也相当于非惊群的. 之所以这样实现是可以利用条件变量实现线程间信号量机制, 这比进程间单纯使用信号量的开销更小
...
- 现在考虑IO多路转接在并发场景下的情况, 首先要明白所谓的事件是针对文件描述符fd, 并不是select, epoll. 即只有当fd状态发生变化后, 才会引发select, epoll. 由这一点可以事先得出几个结论出来:
    1. 不同线程下调用select时注册不同fd时, 某个fd发生变化后,并不会触发所有的select
    2. 不同线程下不同的epoll实例注册不同fd时,情况和select一样
> 可以写程序来验证(select):
...
thread_local int main_fds[4] = {-1};
static const char* pipes[4] = {
	"a.pipe",
	"b.pipe",
	"c.pipe",
	"d.pipe",
...
static void sub_thread(const char* pipe){
	fprintf(stdout,"tid[%ld]---%s\n",pthread_self(), pipe); 
	sleep(1);
	auto fd = open(pipe, O_RDONLY | O_NONBLOCK);
	...
	while(1){
		FD_ZERO(&s);
		FD_SET(fd,&s);
		num = select(fd + 1, &s, nullptr, nullptr, nullptr);        // __code_select
		fprintf(stdout, "tid[%ld]---%s num:%d\n",pthread_self(), pipe, num); 
		if(num < 0){
			if(errno == EINTR){
				continue;
			}
		...
		if(num){
			if(FD_ISSET(fd, &s)){
				char buf[0x100] = {0};
				auto rn = read(fd, buf, 0x100);
				if(rn > 0){
					write(STDOUT_FILENO, "receive from ", 13);
					write(STDOUT_FILENO, pipe, __builtin_strlen(pipe));
...
int main(int args, char** argv){
	for(int i = -1; ++i < 4;){
		main_fds[i] = open(pipes[i], O_RDWR | O_NONBLOCK);  
		thread(sub_thread, pipes[i]).detach();              
	}
	...
	while(1){
		int a;
		string str;
		std::cout << "@input index pipe:";
		cin >> a;
		std::cout << "@content:";
		cin >> str;
		if(cin){
			auto buf = str.c_str();
			write(main_fds[a % 4], buf, str.size());
			sleep(1);
		}else{
			cin.clear();
		}
...
<img src="./assets/14_08.gif" />

<br/>

> 上述程序测试的是多线程下不同select监听不同fd的情况, 忽略了某些出错情况的判断. 主线程中非阻塞打开所有管道, 并将标准输入的内容转发到管道中, 而对应的子线程由于监听了不同的管道会被唤醒. 可以发现事件是由fd来驱动select的. 程序中保证了在运行过程中写端的管道始终是打开的(`主线程中`). 
> 
>  PS: 前面测试`epoll`时, 当管道的写端被关闭后, `epoll_wait`会被`EPOLLHUP`不停唤醒, 而`epoll`必定被fd(`管道文件描述符`)状态变化所唤醒, 所以`EPOLLHUP`的产生可能与fd状态有关, 所以该案例中为了在测试逻辑上的正确性, 应该保证管道写端始终是打开的. 
>
> 上述程序修改一下: 将select替换为epoll(`每条线程都创建自己的epoll`)
...
static void sub_thread(const char* pipe){
	...
	int num = 0;
	auto efd = epoll_create(1);
	struct epoll_event e, ebuf;
	e.data.fd = fd;
	e.events = EPOLLIN;
	epoll_ctl(efd, EPOLL_CTL_ADD, fd, &e);

	while(1){
		num = epoll_wait(efd, &ebuf, 1, -1); 
		print_e(ebuf);
		fprintf(stdout, "tid[%ld]---%s num:%d\n",pthread_self(), pipe, num); 
		if(num < 0){
			if(errno == EINTR){
        ...
        // 不考虑num为0的情况(正常逻辑会由超时造成, 但在demo中设置了永久阻塞)
		if(num){
			if(ebuf.events & EPOLLIN){
				char buf[0x100] = {0};
				ssize_t rn = 0;
				while(1){
					rn = read(ebuf.data.fd, buf, 0x100);
					if(rn > 0){
						write(STDOUT_FILENO, "receive from ", 13);
						write(STDOUT_FILENO, pipe, __builtin_strlen(pipe));
						write(STDOUT_FILENO, ":\t", 2);
						...
						if(errno == EAGAIN){ // 管道中没有内容了, 回去继续监听
							break;
						}
						perror("read()");
						close(efd);
						...
						epoll_ctl(efd, EPOLL_CTL_DEL, ebuf.data.fd);
						break;
					}
					rn = 0;
...
<img src="./assets/14_09.gif" />

---



- 上面的2个测试指在说明select和epoll本质由fd驱动, 接下来探究一下同一fd在并发下slect和epoll的行为. 首先对于select来说:
    1. 当同一个fd被多个线程的select关联时, 从逻辑上来推测应该会唤醒所有的select, 毕竟fd状态改变时会驱动select, 那这里就涉及到了select是不是惊群唤醒.
    2. fd底层对应文件系统中的唯一文件, 所以要测试不同的线程分别打开相同文件时, select的行为

<br/>

- 对于epoll的测试要复杂一些. 因为epoll是一个对象, 所以笔者从这几点来测试:
    1. 同一epoll实例在多线程中调用`epoll_ctl`是否是安全的
    2. 多线程共享epoll实例时, 相同fd的行为
    3. 多线程共享epoll实例时, 不同fd的行为
    4. 多线程不同epoll实例时, 相同fd的行为


<br/>

- <font size = 5>并发下select相同fd的行为</font>
...
static int fd_a, fd_b, fd_c, fd_d;
static fd_set set;
static int fds[4];

static void keep_pipe_write(void){      
	open("a.pipe", O_WRONLY);
	open("b.pipe", O_WRONLY);
	open("c.pipe", O_WRONLY);
	open("d.pipe", O_WRONLY);       // 当有读者时, 可以阻塞打开
}

int main(int args, char** argv){
	fd_a = open("a.pipe", O_RDONLY | O_NONBLOCK);
	fd_b = open("b.pipe", O_RDONLY | O_NONBLOCK);
	fd_c = open("c.pipe", O_RDONLY | O_NONBLOCK);
	fd_d = open("d.pipe", O_RDONLY | O_NONBLOCK);
	fds[0] = fd_a;
	fds[1] = fd_b;
	fds[2] = fd_c;
	fds[3] = fd_d;                      // __code_0
                                        ...
                                        // __code_1
	std::cout << hex << fds[0] << "\t" << fds[1] << "\t" << fds[2] << "\t" << fds[3] << endl;

	keep_pipe_write();                  // __code_keep_pipe_write
	...
	for(int i = -1; ++i < 10;){
		thread([]{
			...
			fd_set set;                 // __code_local

			int res = 0;
			char buf[0x100] = {0};
			ssize_t rn = 0;
			while(1){                   // __code_while_0
				FD_ZERO(&set);
				FD_SET(fd_a, &set);
				FD_SET(fd_b, &set);
				FD_SET(fd_c, &set);
				FD_SET(fd_d, &set);     // __code_add_fd

                                        
                                        // __code_call_select
				res = select(fd_d + 1, &set, nullptr, nullptr, nullptr);

				if(res < 0){            // __code_select_err
					perror("select()");
					pthread_exit(nullptr);
				}

				// res == 0的情况是超时, 在本例中不会出现



                                        // __code_print_fd_bit
				uint8_t bit_buf[128] = {0};
				__builtin_memcpy(bit_buf, reinterpret_cast<void*>(&set), 128);
				fprintf(stdout,"tid[%ld] wake is:%d bit:%x-%x-%x-%x-%x-%x-%x-%x\n", 
						pthread_self(), 
						res, 
						bit_buf[7],
						bit_buf[6],
						...
						bit_buf[0]
                                        ...
                                        // __code_hand_select_fds
				for(int i = -1; ++i < 4 && res;){
					if(FD_ISSET(fds[i], &set)){
						while(1){
							rn = read(fds[i], buf, 100);
							if(rn < 0){
								if(errno == EAGAIN){
									// 没有数据, 继续下一个fd
									puts("eagain\n");
							...
							fprintf(stdout,"tid[%ld] read data:%s\n",pthread_self(), buf); 
							__builtin_memset(buf, 0, 0x100);
						}
...
> 先对上述代码做一下解释:
>   
> `__code_0`: 程序启动后, 打开4个管道文件,并记录在全局变量中以作为后续子线程共享的文件描述符
>
> `__code_1`: 以16进制打印出4个fd的值(3, 4, 5, 6), 其实表示的就是`fd_set`中的第几个bit位
>
> `__code_keep_pipe_write`: 前面提到过在shell中测试的时候, 命令完毕后, 管道是自动关闭了, 这会导致fd在IO出现为0的情况(`写端断开并且没有内容了`), 所以这里为了避免这种情况, 在程序直接以写方式打开这4个管道, 保持管道写端不断开, 然后在shell中向管道写入内容
>
> `__code_local`: 10个子线程内部定义独立的`fd_set`
>
> `__code_while_0`: 开始进入`select`的监听流程
>
> `__code_add_fd`: 调用select前初始化set, 并添加4个管道的文件描述符
>
> `__code_call_select`: 开始调用select函数, 注意这里取的`maxfd`以要监听的4个管道中最大的那个再+1, 直接传递的是`fd_d`的值+1, 因为Unix的实现中, 文件描述符默认是从小到大产生的, 所以`fd_d`作为监听的4个管道中最后调用的, 所以这里传值是没有问题的
>
> `__code_select_err`: select调用出错误, 直接退出线程
>
> `__code_print_fd_bit`: 当select正常唤醒后, 查看一下当前线程记录的文件描述符. 在前面的学习中已经知道`struct fd_set`的结构, 即set变量的本质就是一块连续的128字节的内存空间, 所以这里直接内存拷贝到`bit_buf`中, 然后以16进制打印出`set`中记录的bit位. 通过这个打印可以看出是哪个fd响应了
>
> `__code_hand_select_fds`: 处理select的返回, 这里要特别注意read后EAGAIN, 因为是循环读取管道, 由于写端是未关闭的, 所以当管道中没有内容后, 会报这个错误, 此刻应该回到`__code_while_0`
>
>
> 先对上述代码分述推测, 当使用局部文件描述符集合时(`__code_local`), 从代码的层面先去分析: select内部发现相关的某些fd状态发生改变时, 会将数据分别拷贝到不同的内存中(`线程的局部变量set`), 要使这个过程是线程安全的, 则逻辑上有2种可能:
>   1. 只有1条select醒来, 即非惊群唤醒
>   2. 如果1不成立, 则是惊群唤醒, 不同线程的select 从内核读取结果的过程必定是独占的. 用线程A的selectA, 线程B的selectB, fdA,fdB状态发生变化来推测一下详细的流程:
>       - 内核发现fdA, fdB可以IO了
>       - 内核唤醒A和B
>       - 假设selectA先抢占开始读取内核数据
>       - selectA独占读数据
>       - selectA读取数据完毕后, 放开独占, 此刻selectB立即抢占到从内核拿, 但此刻数据已经被A拿走了
>       - 最后造成的情况是selectA返回2, selectB返回0
>
> 测试结果如下:

---

<img src="./assets/14_10.gif" />
...
>   1. 先启动程序, 输出4个文件描述符的16进制(3, 4, 5, 6)
>   2. 先在shell中向管道`c.pipe`中写入了数据(`date命令`),  明显看到10个线程中, 有3个线程同时被唤醒, 但只有1个读取了管道的内容, 其余的2个管道进行read后, 已经没有内容, 所以报了`EAGAIN`
>   3. 继续通过shell同时向4个管道中写入了下标`pipes=({a..d}.pipe);for i in {1..4}; do echo $i > ${pipes[$i]} &; done`. 结果是同样的原理, 多作4个线程被唤醒了, 但内容都获取到了, 没有获取内容的线程产生了`EAGAIN`.
>
> 通过测试发现上述推测是不成立的. 但可以知道select唤醒模式是惊群的. 只是取数据的过程是竞争的, 所以从本质上来说是不安全的. 猜测应该是这样的:
>   1. 当4个fd状态发生变化时, 内核只是记录下了4个fd可以read了
>   2. 10条线程在select处被唤醒等待调度, 
>   3. 由于调度机制, 可能有6条线程直接醒来, 其余4条线程还在就绪队列
>   4. 6条线程竞争的去取内核记录的4个fd, 然后向自己set空间中回填
>   5. 当select返回后, 这6条线程中有的线程会将管道中的数据读取出来,内核才会标记fd的状态不能read了. 然后其他线程再读时就直接`EAGAIN`了
>   6. 等到6条线程将数据全部取完, 那些还在就绪队列中的线程从select醒来, 检查时已经没有任何可IO的fd了, 就又睡眠了
>
> 这里只是笔者的推断, 因为demo已经证明了select是惊群的, 所以不会出现内核只随机唤醒6条线程, 毕竟产生了4个可以IO的fd, 所以逻辑上是10条线程全部唤醒了, 只是调度机制和前后数据消费的时间差, 那4条又挂起了
> 
> 当使用全局变量的set时, 应该是不安全的, 因为相当于多个线程同时向同一块内存做操作
>
> select是惊群唤醒, 从逻辑上来说并不安全, 以demo来说多条线程同时醒抢到了fd, 然后进行了read,但实际开发中select是安全的, 因为select监听的fd一般都是能造成阻塞的低速设备, 如管道, socket, 字符设备, 打印机等. 当这些fd在不同的线程中同时由select唤醒后,下一步即使是多线程对同一fd做IO操作, IO本身这个过程是安全的. 那些失败的IO会可能会返回相应的错误. 所以才说是安全的. 但不管结果怎样, 应该始终从严谨的角度来看待select, 并且当select监听大量的fd时(`小于1024`), 并发场景下select会出现大量的IO错误, 从效率上来看非常耗性能的
>
> 解决方案:
> 无法做到避免惊群唤醒, 因为这是系统调用的select控制的, 在用户层面能做的只有: 判断IO的读取结果. 有些系统中的管道设备可以调用stat函数查看一下当前size的值, 若有值则表示有数据, 可以利用这种特性在IO返回后调用stat函数, 但这是不可取的, 原因是:
>   1. 不兼容, 有的系统管道文件的size是未定义的
>   2. 虽然避免了read的系统调用, 但事实产生了stat的系统调用, 并且即使是调用stat获取当前的size后有数据, 也不能保证自己再去IO时, 管道中还有数据, 因为这2步不是原子的. 但在多进程环境下可以使用文件锁:
>       - 从select醒来后尝试加锁, 失败则读一下个fd. 
>       - 前提是多进程环境, 并且只有当文件锁的效率远高于read出错时才有效果

```txt
select(...)     // fd

auto res = ftrylock(fd) 
    if(res < 0){
        继续读取下一个fd
    }
...
funlock()
```
> 这样做会使逻辑更加复杂,不管怎样都会进行至少1次的系统调用, 即使ftrylock函数的调用速度比read快也不会解决根本问题
...
- <font size = 5>并发下select相同fd的行为</font>
- 可以推测的是select还是会唤醒多条线程, 因为底层相当于是同一文件. 把上面的代码作这样的修改就可测试:
...
thread_local static int fd_a;
thread_local static int fd_b;
thread_local static int fd_c;
thread_local static int fd_d;
thread_local static fd_set set;
thread_local static int fds[4];

static void keep_pipe_write(void){
	open("a.pipe", O_WRONLY);
	open("b.pipe", O_WRONLY);
	open("c.pipe", O_WRONLY);
	open("d.pipe", O_WRONLY);
}
...
int main(int args, char** argv){
	for(int i = -1; ++i < 10;){
		thread([]{
			...
			fds[2] = fd_c;
			fds[3] = fd_d;
			fprintf(stdout,"tid[%ld]\n%x--%x--%x--%x\n", pthread_self(), fds[0], fds[1], fds[2], fds[3]); 
...
> 该程序是对上个程序的稍作修改, 直接将原来定义的变量类型替换成`thread_local`, 调整了原来的执行流程, 先创建线程保证4个管道读打开, 然后主线程再以阻塞写打开(`因为有了读者不会阻塞`). 可以推测出结果和上个程序一样, 因为不管怎么改, fd对应的文件在文件系统中只有一份
...
- <font size=5>共享epoll实例的`epoll_ctl`是否线程安全</font>
- 首先对于`epoll_ctl`, 该函数的作用是将fd和事件注册到红黑树中, 如果在插入过程中不是线程安全的则将可能破坏结构. 导致的结果就是`epoll`调用异常. 下面的案例是测试同一个epoll实例在并发下的行为
...
int main(int args, char** argv){
	...
	fd_a = open("a.pipe", O_NONBLOCK | O_RDONLY);

	struct epoll_event e;
	e.data.fd = fd_a;
	e.events  = EPOLLIN;
	epoll_ctl(efd, EPOLL_CTL_ADD, fd_a, &e);

	for(int i = -1; ++i < 10;){
		thread([](int i){
			fprintf(stdout, "tid[%ld] idx:%c\n", pthread_self(), 'a' + i);
			int fd = open(pipes[i], O_NONBLOCK | O_RDONLY);
			...
			struct epoll_event ebuf[4] = {0};
		
			auto res = epoll_wait(efd, ebuf, 4, -1);
			if(res < 0){
				if(errno == EINTR){
					// 这里是假错, 当循环epoll_wait时, 这里要继续
				}
				perror("epoll_wait");
			}
			fprintf(stdout, "tid[%ld] wake num:%d\n", pthread_self(), res);
			for(int i = -1; ++i < res;){
				auto e = ebuf[i];
                if(e.events & EPOLLIN){
                    char buf[0x100] = {0};
                    read(e.data.fd, buf, 0x100);
                    fprintf(stdout, "tid[%ld] read data:%s\n", pthread_self(), buf);
                }
                if(e.events & EPOLLHUP){
                    // 这里要移除, 因为当前测试的是管道, 在shell中写入管道后将直接关闭
                    // 导致epoll_wait发现缓冲区没有内容时, 一直触发  EPOLLHUP事件. 这里在做测试不移除
                    // 当实际场景中是socket时, 也是同样的做法, 要移除连接
                    //epoll_ctl(efd, EPOLL_CTL_DEL, e.dada.fd, nullptr);
                }
...
> 该测试中, 10条子线程使用了同一个epoll实例. 但为了模拟多线程并发对红黑树的修改, 在每条线程开始时, 会循环2000次做删除插入的操作, 尽可能的并发修改树结构. 若这个过程是线程安全的则`epoll_wait`的调用是没有问题的. 测试结果如下:
...
## $(($RANDOM % 10)) 产生 0~9的随机数
## ${arr[xx]} 则是从数组中根据随机数取出文件名的前缀
echo $RANDOM > ${arr[$(($RANDOM % 10))]}.pipe

### 这里有个小问题, shell中的数组是从1开始, 但这里产生随机数是0~9, 所以测试中的截图有这个小问题,但对测试没有影响
```

<br/>


> 通过测试发现, 即使没有对`epoll_ctl`上锁, 它也是线程安全的. 同时可以看到`epoll_wait`默认情况下是惊群的. 图中是随机一个 xx.pipe 文件向其中写入内容, 当先运往本程序后, 等待一定时间再写入管道时`epoll_ctl`的插入操作已经将10个fd注册到树中了. 最后呈现的结果就是`epoll_wait`是惊群的 

<br/>


- <font size=5>不同epoll实例同一fd并发</font>
- 可以推测到, 也是惊群的. 
...
int main(int args, char** argv){
	...
	for(int i = -1; ++i < 10;){
		thread([](int i){
			...
			struct epoll_event ebuf[1] = {0};
		
			auto res = epoll_wait(efd, ebuf, 1, -1);
			if(res < 0){
				if(errno == EINTR){
					// 这里是假错, 当循环epoll_wait时, 这里要继续
				}
...
> 从这一系列的测试中可以发现: 事件的本质是fd关联的底层文件, `epoll_ctl`的系统调用也只是注册了对这个事件的回调, 也就是说事件对epoll来说是1对多的关系, 即同一个事件会唤醒多个epoll实例.  同一epoll注册不同fd时更容易理解,只要fd的状态发生改变, 它被关联的epoll就会被唤醒. 并且可以断定不同线程调用同一epoll实例的`epoll_wait`时, 内核所要唤醒的并不是表面上的epoll实例, 而是阻塞在所有线程上的这个epoll(`回顾epoll结构中的链表, 这个链表中所记录的就是所有在该epoll上的就绪线程`). 现在要想一下怎么避免惊群的问题. 


<br/>

### epoll避免惊群
- 笔者只考虑同一epoll实例并发的情况, 要做到的效果就是当前线程不进行多余的IO操作: 即epoll被惊群唤醒后, 线程做到当前fd有数据时才读, 没有数据立即遍历到下一个fd做同样的判断. 解决方案是对当fd操作的过程做临界区. 则先尝试加锁, 失败后继续遍历到下一个fd. 再次说一点, 以管道为例, 即使多个线程同时非阻塞读取, 也只有1个能成功, 其他的会失败, 所以从理论上来说不用特意做这种加锁的判断来保证应用层面的互斥. 除非判断锁的效率就远高于IO出错的效率. 但这里笔者只是提出一种解决方案.  注意这个锁不能是文件锁(`虽然管道也可以加锁`), 因为当前环境是多线程, 文件锁没有效果, 所以可以使用`mutex`或`CAS`, 下面的案例使用了效率较高了CAS, 因为临界区的部分代码很少, 否则应该选用互斥锁

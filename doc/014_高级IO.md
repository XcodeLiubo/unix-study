...
> 上述代码用宏区分当前环境是多线程还是多进程. 多线程时使用了mutex(<font color=green>C++标准库的mutex调用到POSIX底层phread相关的互斥锁</font>).多进程使用了文件锁. 在临界区(`LOCK与UN_LOCK`)所要做的是从文件中读取内容做加1操作后写入到文件, 其中特意睡眠了1秒. 从测试打印中可以发现, 当解锁后只有1条线程或进程被唤醒. 这当然是符合常理的, 但不能从这一点来推断出文件锁内部是不是惊群了, 可能唤醒了所有的进程, 然后只有1条进程抢到了锁, 其他进程则抢到锁失败导致又睡眠. 但互斥锁一定是不惊群的, 因为在第`11_12`章线程的章节中, 已经分析了源码, 互斥锁内部调用了`futex_wait(Linux)`, 这个系统调用详细说明了, 只会唤醒1条线程. 惊群的效果在互斥锁中通过条件变量的接口可以很明确的测试出来`pthread_cond_broadcast`. 文件锁是不是惊群的笔者自己还不能写代码确定, 但手册中说现在的flock函数已经实现为了系统调用和`fcntl`上锁是分开的, 既然是系统调用, 则内核应该是效率较高的非惊群的, 同时笔者也在网上查阅了资料, 像nginx使用过文件锁来防止惊群, 并且以笔者对惊群的定义, 认为文件锁也是非惊群的
...
- 现在考虑IO多路转接在并发场景下的情况, 要关注的是以下几点:
    1. 相关系统调用(`select poll epoll`)的行为本身是不是安全的. 即不添加任何临界区时, 多线程情况下被同一fd唤醒时的行为
    2. 若本身安全,则是不是惊群的
    3. 若本身不安全,则解决问题的方案有哪些
- 针对于这些问题, 最主要的是怎么写测试程序? 对于select来说, 应该在多线程下测试:
    1. 同一fd在不同线程下调用select
    2. 同一文件不同fd调用select
- poll函数的原理不做测试, 因为它和select是一样的机制. 对于epoll来说:
    1. 多线程中对同一epoll实例做`epoll_ctl`时并发操作是不是安全的
    2. 多线程中监听同一epoll实例
    3. 多线程中不同epoll实例中所注册的同一fd
    4. 我线程中不同epoll实例中所注册的同一文件

<br/>

- <font size = 5>多线程下同一fd下调用select</font>
...
int main(int args, char** argv){
	...
	for(int i = -1; ++i < 10;){
		thread([]{
			...
			if(res == 0){
				fprintf(stdout,"tid[%ld] wake is zero\n", pthread_self()); 
				return;
			}
			fprintf(stdout,"tid[%ld] wake is:%d\n", pthread_self(), res); 
			char buf[0x100] = {0};
			for(int i = -1; ++i < res;){
...
> 当使用局部文件描述符集合时(`__code_local`), 从代码的层面先去分析: select内部发现相关的某些fd状态发生改变时, 会将数据分别拷贝到不同的内存中(`线程的局部变量set`), 这个过程要保证是线程安全的, 则有2种可能:
...
> 这里测试的流程是:
>   1. 先执行shell命令并发的向4个管道中写入数据, 所以这个先后是不确定的
>   2. 再运行程序, 可以肯定的是内核已经标记4个fd的状态都可以read了.
>   
> 最后测试的结果却是所有的select都返回了4, 也就是说上述推测是不成立的. 但可以知道select唤醒模式是惊群的. 只是取数据的过程是竞争的, 所以从本质上来说是不安全的. 猜测应该是这样的, 内核只是记录下了4个fd可以read了, 当4条线程调用select时, 内核直接唤醒它们就不管了. 它们竞争的去读取设置fd到自己的线程空间中.
> 
> 至于最后程序执行没有出错, 是因为线程循环取fd读取数据时, 管道本身是安全的. 那些打印结果是`wake is4: read data:`的线程其实已经发生了`eof`, 笔者没有在代码中体现出来. 同时可以推测的是: 当使用全局变量的set时, 应该是不安全的, 因为相当于多个线程同时向同一块内存做操作
>
> 从测试中可以发现select是惊群的, 而且相对来说是不安全的. 但实际开发中select是安全的, 因为select监听的fd一般都是能造成阻塞的低速设备, 如管道, socket, 字符设备, 打印机等. 当这些fd在不同的线程中同时由select唤醒后,下一步即使是多线程对同一fd做IO操作, IO本身这个过程是安全的. 那些失败的IO会可能会返回相应的错误或eof. 所以说是安全的. 但不管结果怎样, 应该始终从严谨的角度来看待select, 并且当select监听大量的fd时(`小于1024`), 并发场景下select会出现大量的IO错误或eof, 从效率上来看非常耗性能的
>
> 解决方案:
> 无法做到避免惊群唤醒, 因为这是系统调用的select控制的, 在用户层面能做的只有: 判断IO的读取结果. 有些系统中的管道设备可以调用stat函数查看一下当前size的值, 若有值则表示有数据, 可以利用这种特性在IO返回后调用stat函数, 但这是不可取的, 主要是:
>   1. 不兼容, 有的系统管道文件的size是未定义的
>   2. 虽然避免了read的系统调用, 但事实产生了stat的系统调用, 并且即使是调用stat获取当前的size后有数据, 也不能保证自己再去IO时, 管道中还有数据, 因为这2步不是原子的. 但其实可以利用这一点自己来实现临界区:
...
unlock()
```
> 这样做会使逻辑更加复杂,不管怎样都会进行至少1次的系统调用, 即使stat函数的调用速度比read快也不会解决根本问题


<br/>


- <font size = 5>多线程下同一文件下调用select</font>
...
thread_local static int fd_a;       // 线程变量 
